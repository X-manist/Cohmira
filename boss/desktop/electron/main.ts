import { app, BrowserWindow, ipcMain, protocol, nativeImage, shell, clipboard, dialog, net, type WebContents } from 'electron'
...
import {
  ...
  getMcpOAuthStatus,
  getGooseMcpExtensionOverrides,
  type McpServerConfig,
} from './core/mcpStore';
import { listMcpTools, callMcpTool } from './core/mcpRuntime';
import { handleBeavLocalBridgeRequest } from './core/beavLocalBridge';
import {
  buildGooseEndpoints,
  GooseBridgeService,
  ...
  type GooseRuntimeEventEnvelope,
} from './core/gooseBridge';
...
let gooseBridgeListenersAttached = false;
const gooseBridgeService = new GooseBridgeService();
const activeGooseChatRequests = new Map<string, string>();
...
async function shutdownBackgroundServices(): Promise<void> {
  ...
  appShutdownPromise = (async () => {
    ...
    await Promise.allSettled([
      ...
      capture('goose-bridge', () => gooseBridgeService.stop()),
    ]);
...
ipcMain.handle('mcp:list-tools', async (_, payload: { server?: McpServerConfig }) => {
  try {
    if (!payload?.server) {
      return { success: false, error: 'server is required', tools: [] };
    }
    const tools = await listMcpTools(payload.server);
    return { success: true, tools, response: { tools } };
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    return { success: false, error: message, tools: [] };
  }
});
...
ipcMain.handle('mcp:call', async (_, payload: {
  ...
  try {
    ...
    const response = await callMcpTool(payload.server, toolName, args);
...
function attachGooseBridgeListeners(): void {
  if (gooseBridgeListenersAttached) return;
  gooseBridgeListenersAttached = true;
  gooseBridgeService.on('event', (payload) => {
    for (const browserWindow of BrowserWindow.getAllWindows()) {
      browserWindow.webContents.send('goose:event', payload);
    }
  });
  gooseBridgeService.on('log', (payload) => {
    for (const browserWindow of BrowserWindow.getAllWindows()) {
      browserWindow.webContents.send('goose:log', payload);
    }
  });
  gooseBridgeService.on('exit', (payload) => {
    for (const browserWindow of BrowserWindow.getAllWindows()) {
      browserWindow.webContents.send('goose:exit', payload);
    }
  });
  gooseBridgeService.on('error-event', (payload) => {
    for (const browserWindow of BrowserWindow.getAllWindows()) {
      browserWindow.webContents.send('goose:error', payload);
    }
  });
}

function sendRuntimeEvent(sender: WebContents, payload: GooseRuntimeEventEnvelope): void {
  sender.send('runtime:event', payload);
}

function shouldUseGooseRuntimeForSession(metadata: Record<string, unknown>): boolean {
  const contextType = String(metadata.contextType || '').trim().toLowerCase();
  const agentRuntime = String(metadata.agentRuntime || metadata.runtimeBridge || '').trim().toLowerCase();
  return agentRuntime === 'goose' || contextType === 'redclaw';
}

function resolveGooseOpenAiEndpoint(baseUrl: string): { host?: string; basePath?: string; baseUrl?: string } {
  const normalized = normalizeApiBaseUrl(String(baseUrl || '').trim());
  return splitGooseOpenAiCompatibleEndpoint(normalized);
}

function resolveGooseHomeDir(): string {
  return path.join(app.getPath('userData'), 'goose');
}

function resolveGoosedExecutablePath(): string | undefined {
  const executableName = process.platform === 'win32' ? 'goosed.exe' : 'goosed';
  const explicit = String(process.env.GOOSE_EXECUTABLE_PATH || '').trim();
  ...
  const configuredBuildTarget = String(process.env.GOOSE_BUILD_TARGET || '').trim();
  const nativeBuildTarget = process.platform === 'win32'
    ? process.arch === 'arm64' ? 'aarch64-pc-windows-msvc' : 'x86_64-pc-windows-msvc'
    : process.platform === 'darwin'
      ? process.arch === 'arm64' ? 'aarch64-apple-darwin' : 'x86_64-apple-darwin'
      : '';
  const buildTargets = [...new Set([configuredBuildTarget, nativeBuildTarget].filter(Boolean))];
  const sourceBuilds = sourceRoots.flatMap((sourceRoot) => {
    const targetRoot = path.join(sourceRoot, 'target');
    return [
      path.join(targetRoot, 'release', executableName),
      path.join(targetRoot, 'debug', executableName),
      ...buildTargets.flatMap((target) => [
        path.join(targetRoot, target, 'release', executableName),
        path.join(targetRoot, target, 'debug', executableName),
      ]),
    ];
  });
  const candidates = [
    explicit,
    resourcesPath ? path.join(resourcesPath, 'app.asar.unpacked', '.goose-runtime', 'bin', executableName) : '',
    resourcesPath ? path.join(resourcesPath, '.goose-runtime', 'bin', executableName) : '',
    resourcesPath ? path.join(resourcesPath, 'app.asar.unpacked', 'goose', 'bin', executableName) : '',
    resourcesPath ? path.join(resourcesPath, 'goose', 'bin', executableName) : '',
    path.resolve(process.cwd(), '.goose-runtime', 'bin', executableName),
    path.resolve(appPath, '.goose-runtime', 'bin', executableName),
    ...sourceBuilds,
  ].filter(Boolean);
...
function buildGooseRuntimeConfig(input: {
  apiKey?: string;
  baseURL?: string;
  modelName?: string;
}): GooseBridgeConfig {
  const endpoint = resolveGooseOpenAiEndpoint(String(input.baseURL || ''));
  const apiKey = String(input.apiKey || '').trim();
  const model = String(input.modelName || '').trim();
  const gooseHome = resolveGooseHomeDir();
  return {
    executablePath: resolveGoosedExecutablePath(),
    host: '127.0.0.1',
    port: 3000,
    tls: false,
    useSessionEvents: true,
    cwd: getWorkspacePaths().base,
    gooseHome,
    provider: 'openai',
    model,
    env: {
      GOOSE_DISABLE_TELEMETRY: '1',
      GOOSE_PATH_ROOT: gooseHome,
    ...
    extensionOverrides: getGooseMcpExtensionOverrides(),
  };
}

async function isGooseRuntimeReady(config: GooseBridgeConfig): Promise<boolean> {
  try {
    const response = await fetch(buildGooseEndpoints(config).statusUrl);
    return response.ok;
  } catch {
    return false;
  }
...
async function waitForGooseRuntimeReady(config: GooseBridgeConfig, timeoutMs = 25000): Promise<void> {
  ...
  throw new Error(lastError || 'goosed /status did not become ready');
}

async function ensureGooseRuntimeAvailable(config: GooseBridgeConfig): Promise<void> {
  if (await isGooseRuntimeReady(config)) return;
  gooseBridgeService.start(config);
  try {
    await waitForGooseRuntimeReady(config);
  } catch (error) {
    const status = gooseBridgeService.getStatus();
    const detail = error instanceof Error ? error.message : String(error);
    throw new Error(`Goose runtime 不可用：请先运行 npm run goose:build，或通过 GOOSE_EXECUTABLE_PATH 指定 goosed。当前命令：${status.command.command} ${status.command.args.join(' ')}；${detail}`);
  }
}

async function sendMessageWithGooseRuntime(input: {
  sender: WebContents;
  sessionId: string;
  message: string;
  requestId?: string;
  ...
  attachGooseBridgeListeners();
  const requestId = String(input.requestId || randomUUID()).trim();
  activeGooseChatRequests.set(input.sessionId, requestId);
  const adapter = new GooseChatEventAdapter({
    sessionId: input.sessionId,
    requestId,
    runtimeMode: 'goose',
  });
  const onGooseEvent = (event: unknown) => {
    for (const runtimeEvent of adapter.accept(event as GooseNormalizedEvent)) {
      sendRuntimeEvent(input.sender, runtimeEvent);
    }
  };

  gooseBridgeService.on('event', onGooseEvent);
  sendRuntimeEvent(input.sender, {
    eventType: 'runtime:stream-start',
    sessionId: input.sessionId,
    taskId: requestId,
    runtimeId: `goose:${requestId}`,
    payload: {
      phase: 'thinking',
      runtimeMode: 'goose',
    },
  ...
  try {
    const runtimeConfig = buildGooseRuntimeConfig(input.modelConfig || {});
    await ensureGooseRuntimeAvailable(runtimeConfig);
    await gooseBridgeService.sendMessage({
      sessionId: input.sessionId,
      requestId,
      text: input.message,
      config: runtimeConfig,
    });
    const assistantText = adapter.getAssistantText().trim();
    if (assistantText) {
      addChatMessage({
        id: `msg_goose_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`,
        session_id: input.sessionId,
        role: 'assistant',
        content: assistantText,
      });
    }
    for (const runtimeEvent of adapter.finish('stop')) {
      sendRuntimeEvent(input.sender, runtimeEvent);
    }
  } catch (error) {
    const cancelled = error instanceof Error && error.name === 'GooseBridgeTaskCancelledError';
    if (cancelled) {
      sendRuntimeEvent(input.sender, {
        ...
        payload: {
          ...
          summary: 'Goose runtime cancelled',
          payload: {},
        },
        timestamp: Date.now(),
      });
    ...
    for (const runtimeEvent of adapter.fail(error)) {
      sendRuntimeEvent(input.sender, runtimeEvent);
    }
    throw error;
  } finally {
    if (activeGooseChatRequests.get(input.sessionId) === requestId) {
      activeGooseChatRequests.delete(input.sessionId);
    }
    gooseBridgeService.off('event', onGooseEvent);
  }
}

ipcMain.handle('goose:status', async () => {
  attachGooseBridgeListeners();
  return { success: true, status: gooseBridgeService.getStatus() };
});

ipcMain.handle('goose:start', async (_, payload?: { config?: GooseBridgeConfig }) => {
  try {
    attachGooseBridgeListeners();
    const status = gooseBridgeService.start(payload?.config);
    return { success: true, status };
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    return { success: false, error: message, status: gooseBridgeService.getStatus() };
  }
});

ipcMain.handle('goose:stop', async () => {
  try {
    const status = gooseBridgeService.stop();
    return { success: true, status };
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    return { success: false, error: message, status: gooseBridgeService.getStatus() };
...
ipcMain.handle('goose:send-message', async (_, payload?: {
  sessionId?: string;
  requestId?: string;
  text?: string;
  message?: Record<string, unknown>;
  overrideConversation?: Array<Record<string, unknown>>;
  config?: GooseBridgeConfig;
}) => {
  try {
    attachGooseBridgeListeners();
    const result = await gooseBridgeService.sendMessage({
      sessionId: payload?.sessionId,
      requestId: payload?.requestId,
      text: payload?.text,
      message: payload?.message,
      overrideConversation: payload?.overrideConversation,
      config: payload?.config,
    });
    return { success: true, result, status: gooseBridgeService.getStatus() };
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    return { success: false, error: message, status: gooseBridgeService.getStatus() };
  }
...
ipcMain.handle('goose:cancel', async (_, payload?: { sessionId?: string; requestId?: string; config?: GooseBridgeConfig }) => {
  try {
    const sessionId = String(payload?.sessionId || '').trim();
    if (!sessionId) {
      return { success: false, error: 'sessionId is required' };
    }
    const result = await gooseBridgeService.cancel(sessionId, payload?.config, payload?.requestId);
    return { success: true, result, status: gooseBridgeService.getStatus() };
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    return { success: false, error: message, status: gooseBridgeService.getStatus() };
...
ipcMain.handle('chat:getOrCreateContextSession', async (_, {
  ...
  if (existingSession) {
    ...
    try {
      ...
      const nextMeta = {
        ...meta,
        contextContent: String(initialContext || ''),
        ...(inputMetadata || {}),
      };
  ...
  const metadata = {
    contextId,
    contextType,
    contextContent: String(initialContext || ''), // 存储初始上下文内容
    isContextBound: true,
    ...(inputMetadata || {}),
  };
...
ipcMain.on('chat:send-message', async (event, { sessionId, message, displayContent, attachment, modelConfig, taskHints }) => {
  ...
  try {
    ...
    if (shouldUseGooseRuntimeForSession(sessionMeta)) {
      await sendMessageWithGooseRuntime({
        sender,
        sessionId,
      ...
      console.log('[chat:send-message] completed via goose runtime', { sessionId });
      return;
    }
...
ipcMain.on('chat:cancel', (_, payload?: { sessionId?: string } | string) => {
    ...
    if (sessionId) {
      ...
      if (shouldUseGooseRuntimeForSession((() => {
        try {
          const session = getChatSession(sessionId);
          return session?.metadata ? JSON.parse(session.metadata) as Record<string, unknown> : {};
        } catch {
          return {};
        }
      })())) {
        const requestId = activeGooseChatRequests.get(sessionId);
        if (requestId) {
          gooseBridgeService.cancelTask(requestId);
        }
        void gooseBridgeService.cancel(sessionId, undefined, requestId).catch((error) => {
          console.warn('[chat:cancel] Goose cancel failed:', error);
        });
      }
      return;
    }
...
function readJsonRequestBody(req: http.IncomingMessage, maxBytes: number): Promise<Record<string, unknown>> {
  return new Promise((resolve, reject) => {
    ...
    req.on('data', (chunk: Buffer | string) => {
      ...
      if (totalBytes > maxBytes) {
        fail(new Error(`Request body is too large; limit=${maxBytes} bytes`));
        req.destroy();
        return;
      }
      chunks.push(buffer);
...
function startHttpServer() {
  ...
  httpServer = http.createServer(async (req, res) => {
    ...
    if (req.method === 'POST' && req.url === '/mcp/beav') {
      try {
        const payload = await readJsonRequestBody(req, 5 * 1024 * 1024);
        const response = await handleBeavLocalBridgeRequest(payload);
