...
export async function generateVideosToMediaLibrary(input: GenerateVideosInput): Promise<GenerateVideosResult> {
    ...
    const configuredEndpoint = String(
        input.endpoint || settings.video_endpoint || REDBOX_OFFICIAL_VIDEO_BASE_URL
    ).trim();
    const endpoint = normalizeApiBaseUrl(
        configuredEndpoint,
        REDBOX_OFFICIAL_VIDEO_BASE_URL
    );
    const redBoxEndpoint = isRedBoxCompatibleEndpoint(endpoint);
    ...
    const configuredModel = String(input.model || settings.video_model || '').trim();
    ...
    if (!endpoint) {
        throw new Error('生视频 Endpoint 未配置。请先配置视频生成 API 源。');
    }
    if (!apiKey) {
        throw new Error('生视频 API Key 未配置。请先配置视频生成 API Key。');
    }
    console.log('[VideoGeneration] auth prepared', {
        endpoint,
        provider: redBoxEndpoint ? 'redbox' : 'openai-compatible',
