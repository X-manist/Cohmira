...
import { MessageSquare, Settings as SettingsIcon, FolderOpen, FileEdit, Plus, Pencil, ChevronDown, ChevronLeft, ChevronRight, Bot, Users, ImagePlus, Sun, Moon, X, Download, Package, AlertCircle, ListTodo, Bell, BarChart3, Share2 } from 'lucide-react';
...
const appLogo = '/cohmira-mark.svg';
...
const NAV_ITEMS: { id: ViewType; label: string; icon: typeof MessageSquare; group?: string }[] = [
  ...
  { id: 'redclaw', label: '运营中枢', icon: Bot },
  { id: 'workboard', label: '任务队列', icon: ListTodo },
  { id: 'knowledge', label: '资料库', icon: FolderOpen },
  { id: 'wander', label: '爆品调研', icon: BarChart3 },
  { id: 'manuscripts', label: '内容稿件', icon: FileEdit },
  { id: 'subjects', label: '博主库', icon: Package },
  { id: 'team', label: '团队成果', icon: Users },
  { id: 'social-accounts', label: '社媒账号', icon: Share2 },
...
function readInitialThemeMode(): ThemeMode {
  ...
  let saved = '';
  try {
    saved = String(window.localStorage.getItem(THEME_STORAGE_KEY) || '').trim().toLowerCase();
  } catch {
    // Fall back to the OS preference when persistent storage is unavailable.
  }
...
export function Layout({ children, currentView, onNavigate, immersiveMode = false, globalNotice = null }: LayoutProps) {
  ...
  const isFixedViewportView = currentView === 'manuscripts' || currentView === 'redclaw';
  ...
  useEffect(() => {
    ...
    try {
      ...
      // The selected theme still applies to the current window.
    }
  ...
  useEffect(() => {
    try {
      ...
      // Keep the current-window layout when persistent storage is unavailable.
    }
  }, [isSidebarCollapsed]);
        ...
        >
          ...
          >
            <div
              ...
              title={appVersion ? `Cohmira Workspace v${appVersion}` : 'Cohmira Workspace'}
            >
              <img src={appLogo} alt="商媒运营助手" className="w-[20px] h-[20px] shrink-0 rounded-[5px]" />
              <span
                className={clsx(
        ...
        >
          ...
          >
            <div className="px-8 pt-6 pb-4 border-b border-border flex items-center justify-between gap-3">
              <h2 className="text-2xl font-semibold text-text-primary">商媒运营助手更新</h2>
              <button
                type="button"
