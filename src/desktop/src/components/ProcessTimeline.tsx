...
const getToolDisplayLabel = (name: string): string => {
  const labelMap: Record<string, string> = {
    ...
    workspace: '工作区操作',
    mediacrawler: '素材采集',
    'operations.mediacrawler': '素材采集',
    'operations.crawler': '素材采集',
    'operations.publish': '内容发布',
    'operations.social': '社媒工具',
    'beav.delegate_video_agent': '视频代理',
    delegate_video_agent: '视频代理',
    openmontage: '视频剪辑',
    'openmontage.video_agent': '视频剪辑',
  };
...
const getToolSummary = (toolName: string, input: unknown): string => {
  ...
  const server = pickText('server', 'mcpServer', 'mcp_server');
  const nestedTool = pickText('tool', 'toolName', 'tool_name', 'name');
  if (server && nestedTool) return `${server}.${nestedTool}`;
  return pickText(
    'action',
    'command',
    'cmd',
...
const buildSummaryItem = (item: ProcessItem): SummaryItem | null => {
  if (item.type === 'phase') {
    ...
    const normalizedTitle = title === 'thinking' ? '正在思考' : title;
    return {
      id: item.id,
      status: item.status,
      label: normalizedTitle || '处理阶段',
      desc: item.content || (item.status === 'running' ? 'Agent 正在执行' : '阶段已完成'),
      output: '',
    };
  }

  if (item.type === 'thought') {
    const content = truncateText(String(item.content || item.title || '').trim(), 500);
    return {
      id: item.id,
      status: item.status,
      label: item.title || '思考过程',
      desc: content || (item.status === 'running' ? '正在整理思路' : '思考已完成'),
      output: content,
    };
  }
  ...
  if (item.type === 'tool-call') {
    ...
    return {
      ...
      output: item.toolData?.output ? truncateText(stringifyValue(item.toolData.output)) : '',
    };
  }
