                            ...
                            <div className="w-full max-w-lg rounded-3xl border border-red-200 bg-red-50/90 p-5 text-left shadow-sm">
                                <div className="flex items-start gap-3">
                                    ...
                                    <div className="min-w-0 flex-1">
                                        <div className="text-sm font-semibold text-status-error">知识库会话初始化失败</div>
                                        <div className="mt-1 text-xs leading-6 text-status-error">
                                            {initError || '宿主没有返回可用会话，请重试。'}
                                        </div>
                                        <button
                                            onClick={() => setReloadToken((value) => value + 1)}
                                            className="mt-4 inline-flex items-center gap-2 rounded-full border border-status-error/30 bg-surface-primary px-3 py-1.5 text-xs font-medium text-status-error transition-colors hover:bg-status-error/10"
                                        >
