                                ...
                                : outgoingTransitions.get(item.id)?.presentation === 'slide'
                                    ...
                                    : outgoingTransitions.get(item.id)?.presentation === 'wipe'
                                        ...
                                        : outgoingTransitions.get(item.id)?.presentation === 'flip'
                                            ? 'flip'
                                            : 'none',
                            direction: outgoingTransitions.get(item.id)?.direction as NonNullable<LegacyTimelineClip['transitionStyle']>['direction'],
...
export function projectDurationMs(project: EditorProjectFile): number {
    return Math.max(
        0,
        ...project.items.map((item) => item.fromMs + item.durationMs),
        ...project.animationLayers.map((layer) => layer.fromMs + layer.durationMs),
    );
}
