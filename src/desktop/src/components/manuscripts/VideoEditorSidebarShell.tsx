...
type SidebarTab<T extends string = string> = {
    id: T;
    label: string;
    icon: LucideIcon;
};

type VideoEditorSidebarShellProps<T extends string> = {
    title: string;
    subtitle: string;
    tabs: SidebarTab<T>[];
    activeTabId: T;
...
export function VideoEditorSidebarShell<T extends string>({
    title,
    subtitle,
    tabs,
    activeTabId,
    ...
    onSelectTab,
    children,
}: VideoEditorSidebarShellProps<T>) {
    return (
