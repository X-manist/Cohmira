...
const SocialAccountsPage = lazy(async () => ({ default: (await import('./pages/SocialAccounts')).SocialAccounts }));
...
export type ViewType = 'chat' | 'team' | 'skills' | 'knowledge' | 'settings' | 'social-accounts' | 'manuscripts' | 'archives' | 'wander' | 'redclaw' | 'media-library' | 'cover-studio' | 'generation-studio' | 'subjects' | 'workboard';
...
function normalizePrimaryView(view: ViewType): ViewType {
  if (view === 'media-library') return 'manuscripts';
  return view === 'chat' || view === 'generation-studio' ? 'redclaw' : view;
}
...
function App() {
  ...
  const [currentView, setCurrentView] = useState<ViewType>('redclaw');
  ...
  const [mountedViews, setMountedViews] = useState<Set<ViewType>>(() => computeMountedViews(['redclaw']));
  ...
  const viewHistoryRef = useRef<ViewType[]>(['redclaw']);
  ...
  useEffect(() => {
    const handleNavigate = (event: Event) => {
      ...
      if (!nextView) return;
      setCurrentView(normalizePrimaryView(nextView));
    };
    const handleIpcNavigate = (_event: unknown, detail?: { view?: ViewType }) => {
      const nextView = detail?.view;
      if (!nextView) return;
      setCurrentView(normalizePrimaryView(nextView));
    };
    ...
    window.ipcRenderer.on('app:navigate', handleIpcNavigate);
    return () => {
      window.removeEventListener(REDBOX_NAVIGATE_EVENT, handleNavigate as EventListener);
      window.ipcRenderer.off('app:navigate', handleIpcNavigate);
    };
  }, []);
  ...
  const navigateToChat = (message: PendingChatMessage) => {
    uiTraceInteraction('app', 'nav_to_chat', { to: 'redclaw' });
  ...
  const navigateToGenerationStudio = (intent: GenerationIntent) => {
    const modeLabel = intent.mode === 'video' ? '视频生成' : '图片生成';
    const sourceLabel = intent.sourceTitle ? `，参考来源：${intent.sourceTitle}` : '';
    uiTraceInteraction('app', 'nav_to_generation_studio', { to: 'redclaw', mode: intent.mode, source: intent.source });
    setPendingRedClawMessage({
      content: `请在运营中枢里发起${modeLabel}任务${sourceLabel}。`,
      displayContent: `发起${modeLabel}`,
  ...
  return (
    <>
      <Layout
        ...
        onNavigate={(view) => setCurrentView(normalizePrimaryView(view))}
      ...
      >
        ...
        {shouldRenderView(mountedViews, currentView, persistentViews, 'social-accounts') && (
          <div className={currentView === 'social-accounts' ? 'h-full min-h-0 flex flex-col' : 'hidden'}>
            <Suspense fallback={currentView === 'social-accounts' ? <ViewLoadingFallback /> : null}>
              <SocialAccountsPage isActive={currentView === 'social-accounts'} />
            </Suspense>
          </div>
        )}
