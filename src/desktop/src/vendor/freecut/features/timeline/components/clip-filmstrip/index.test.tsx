...
import type { UseFilmstripOptions, UseFilmstripResult } from '../../hooks/use-filmstrip';

const useFilmstripMock = vi.hoisted(() => vi.fn((_options: UseFilmstripOptions): UseFilmstripResult => ({
  frames: null,
  isLoading: false,
  isComplete: false,
  progress: 0,
...
const mediaResolverMocks = vi.hoisted(() => ({
  ...
  resolveProxyUrl: vi.fn((_mediaId: string): string | null => null),
}));
...
describe('ClipFilmstrip', () => {
  ...
  it('prioritizes the visible source slice instead of the clip start', () => {
    ...
    expect(priorityWindow).toEqual(expect.objectContaining({
