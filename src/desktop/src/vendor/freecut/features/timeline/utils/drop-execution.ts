...
export async function resolveDroppedMediaEntriesFromExternalFiles({
  ...
  try {
    const handles = entries
      ...
      .filter((handle): handle is FileSystemFileHandle => handle !== undefined);
    if (handles.length !== entries.length) {
      notify.warning('Dropped files do not expose reusable file handles in this browser.');
      return null;
    }
    importedMedia = await importHandlesForPlacement(handles);
  } catch (error) {
