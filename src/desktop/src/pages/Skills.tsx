...
import { Lightbulb, Plus, Pencil, X, Check, FileText, RefreshCw, Power, FolderInput, FolderOpen, PackagePlus, Loader2 } from 'lucide-react';
...
export function Skills({ isActive = true }: { isActive?: boolean }) {
    ...
    const handleImportSkillDirectory = async () => {
        ...
        setImportMessage('');
        try {
            const result = await window.ipcRenderer.skills.importLocal();
            if (result?.canceled) return;
            if (!result?.success) {
                setImportMessage(result?.error || '导入失败');
                return;
            }
            setImportMessage('技能目录已导入');
            await loadSkills();
        } catch (error) {
    ...
    const handleImportPluginDirectory = async () => {
        if (isImportingPlugin) return;
        setIsImportingPlugin(true);
        setImportMessage('');
        try {
            const result = await window.ipcRenderer.plugins.importLocal();
            if (result?.canceled) return;
            if (!result?.success) {
                setImportMessage(result?.error || '插件安装失败');
                return;
            }
            setImportMessage('插件已安装到 ~/.agents/plugins，插件内 skills 已加载');
            await loadSkills();
        } catch (error) {
            setImportMessage(error instanceof Error ? error.message : '插件安装失败');
        } finally {
            setIsImportingPlugin(false);
        }
    };

    const handleOpenPluginRoot = async () => {
        try {
            const result = await window.ipcRenderer.plugins.openRoot();
            if (!result?.success) {
                setImportMessage(result?.error || '无法打开插件目录');
            }
        } catch (error) {
            setImportMessage(error instanceof Error ? error.message : '无法打开插件目录');
        }
    };
            ...
            <div className="w-72 border-r border-border bg-surface-secondary/30 flex flex-col">
                <div className="p-4 border-b border-border flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1">
                        ...
                        <button
                            onClick={() => void handleImportSkillDirectory()}
                            disabled={isImporting}
                            className="p-1.5 text-text-tertiary hover:text-accent-primary hover:bg-surface-primary rounded transition-colors disabled:opacity-50"
                            title="导入本地技能目录"
                        >
                            {isImporting ? <Loader2 className="w-4 h-4 animate-spin" /> : <FolderInput className="w-4 h-4" />}
                        </button>
                        <button
                            onClick={() => void handleImportPluginDirectory()}
                            disabled={isImportingPlugin}
                            className="p-1.5 text-text-tertiary hover:text-accent-primary hover:bg-surface-primary rounded transition-colors disabled:opacity-50"
                            title="安装本地插件（包含 skills、工具与资源）"
                        >
                            {isImportingPlugin ? <Loader2 className="w-4 h-4 animate-spin" /> : <PackagePlus className="w-4 h-4" />}
                        </button>
                        <button
                            onClick={() => void handleOpenPluginRoot()}
                            className="p-1.5 text-text-tertiary hover:text-accent-primary hover:bg-surface-primary rounded transition-colors"
                            title="打开插件安装目录"
                        >
                ...
                {importMessage && (
                    <div className="px-4 py-2 border-b border-border text-[11px] text-text-tertiary">
                        {importMessage}
                    </div>
                )}
                        ...
                        <div className="px-6 py-4 border-b border-border flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                {isEditing ? (
                                    ...
                                    <span className="px-3 py-1.5 text-xs text-text-tertiary border border-border rounded-md">
                                        插件资源只读
                                    </span>
                                ) : (
                                    <button
