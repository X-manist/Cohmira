...
export function MediaLibrary({
    ...
    const focusAsset = useCallback((assetId: string) => {
        ...
        setProjectFilter('');
        setQuery('');
        setExpandedAssetId(normalizedAssetId);
        window.requestAnimationFrame(() => {
            assetCardRefs.current[normalizedAssetId]?.scrollIntoView({
                block: 'center',
                behavior: 'smooth',
            });
        });
    ...
    useEffect(() => {
        if (!isActive) return;
        const applyFocusPayload = (payload: unknown) => {
            const assetId = String((payload && typeof payload === 'object' ? (payload as { assetId?: unknown }).assetId : '') || '').trim();
            if (assetId) {
                focusAsset(assetId);
            }
        };
        const handleFocus = (event: Event) => {
            applyFocusPayload((event as CustomEvent).detail);
        };
        try {
            const pendingRaw = window.localStorage.getItem(MEDIA_LIBRARY_FOCUS_STORAGE_KEY);
            if (pendingRaw) {
                window.localStorage.removeItem(MEDIA_LIBRARY_FOCUS_STORAGE_KEY);
                applyFocusPayload(JSON.parse(pendingRaw));
            }
        } catch {
            // Ignore malformed or unavailable storage.
        }
        window.addEventListener(MEDIA_LIBRARY_FOCUS_EVENT, handleFocus);
        return () => window.removeEventListener(MEDIA_LIBRARY_FOCUS_EVENT, handleFocus);
    }, [focusAsset, isActive]);

    useEffect(() => {
        if (!isActive || !expandedAssetId) return;
        const frame = window.requestAnimationFrame(() => {
            assetCardRefs.current[expandedAssetId]?.scrollIntoView({
                block: 'center',
                behavior: 'smooth',
            });
        });
        return () => window.cancelAnimationFrame(frame);
    }, [assets, expandedAssetId, isActive]);
    ...
    const loadData = useCallback(async () => {
        ...
        setError('');
        try {
            const [mediaOutcome, manuscriptsOutcome] = await Promise.allSettled([
            ...
            if (mediaOutcome.status === 'rejected') {
                const message = mediaOutcome.reason instanceof Error
                    ? mediaOutcome.reason.message
                    : String(mediaOutcome.reason || '未知错误');
                setError(`加载媒体库失败：${message}`);
            } else {
                const mediaResult = mediaOutcome.value;
                if (!mediaResult?.success) {
                    const detail = mediaResult?.error || '后端未返回具体原因';
                    const root = String(mediaResult?.root || '').trim();
                    setError(`加载媒体库失败：${detail}${root ? `（目录：${root}）` : ''}`);
                } else {
            ...
            if (manuscriptsOutcome.status === 'fulfilled') {
                const tree = manuscriptsOutcome.value;
                setManuscripts(flattenManuscripts(Array.isArray(tree) ? tree : []));
            } else {
                console.warn('Failed to load manuscript bindings for media library:', manuscriptsOutcome.reason);
                setManuscripts([]);
            }
        } catch (e) {
            if (requestId !== loadDataRequestRef.current) return;
