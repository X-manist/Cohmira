...
import {
  createRedClawPromptPreset,
  loadRedClawPromptPresets,
  saveRedClawPromptPresets,
  type RedClawPromptPreset,
} from './redclaw/promptPresets';
import { DEFAULT_REDCLAW_PROMPT_PRESETS } from './redclaw/config';
...
import {
  ...
  type SocialToolLoginState,
  type SocialToolsConfig,
  type SocialToolsStatus,
...
const DEFAULT_SEEDANCE_VIDEO_ENDPOINT = 'https://ark.cn-beijing.volces.com/api/plan/v3';
const DEFAULT_SEEDANCE_VIDEO_MODEL = 'doubao-seedance-1.5-pro';
...
type SocialPublisherPlatform = keyof SocialToolsConfig['socialConnection']['accounts'];
...
function normalizeFetchedModels(value: unknown): unknown[] {
  ...
  if (record.success === false) {
    const message = String(record.message || record.error || '拉取模型列表失败').trim();
...
export function Settings({
  ...
  const [formData, setFormData] = useState<SettingsFormData & Record<string, any>>({
    api_endpoint: '',
    api_key: '',
    model_name: '',
    workspace_dir: '',
    ...
    video_endpoint: DEFAULT_SEEDANCE_VIDEO_ENDPOINT,
    video_api_key: '',
    video_model: DEFAULT_SEEDANCE_VIDEO_MODEL,
  ...
  const [redClawPromptPresets, setRedClawPromptPresets] = useState<RedClawPromptPreset[]>(() => loadRedClawPromptPresets());
  ...
  const loadRedclawProfileBundle = useCallback(async (options?: { preserveDraft?: boolean; expectedSpaceId?: string }) => {
    ...
    try {
      ...
      if (requestId !== redclawProfileLoadRequestRef.current) return;
      console.error('Failed to load Cohmira profile bundle', error);
  ...
  const autoFetchedModelsKeyRef = useRef<Record<string, string>>({});
  ...
  const getSourceModelList = useCallback((source: AiSourceConfig) => {
    ...
    for (const raw of [...(source.models || []), source.model]) {
      ...
      merged.set(descriptor.id, normalizeAiModelDescriptors([previous, descriptor])[0] || descriptor);
    }
  ...
  const [socialToolsStatus, setSocialToolsStatus] = useState<SocialToolsStatus>(null);
  const [socialToolsDraft, setSocialToolsDraft] = useState<SocialToolsConfig | null>(null);
  const [socialToolsMessage, setSocialToolsMessage] = useState('');
  const [isSocialToolsLoading, setIsSocialToolsLoading] = useState(false);
  const [isSavingSocialTools, setIsSavingSocialTools] = useState(false);
  const [socialToolLoginState, setSocialToolLoginState] = useState<SocialToolLoginState>({
    running: false,
    message: '',
  });
  const socialToolLoginPollInFlightRef = useRef(false);
  ...
  const isOfficialManagedSource = useCallback((source?: {
    ...
    const presetId = String(source.presetId || '').trim().toLowerCase();
    return sourceId === 'redbox_official_auto' || sourceName === '商媒运营助手官方' || sourceName === 'redbox official' || presetId === 'redbox-official';
  }, []);
  ...
  const handleSetSourceDefaultModel = (sourceId: string, modelId: string) => {
    ...
    updateAiSource(sourceId, (source) => ({
      ...
      modelsMeta: normalizeAiModelDescriptors([
        ...(source.modelsMeta || []),
        ...((source.models || []).map((id) => ({ id }))),
        getSourceModelList(source).find((item) => item.id === normalizedModel)
          || { id: normalizedModel, capabilities: ['chat'] },
      ]),
    }));
  };
  ...
  const handleAddSourceModel = (sourceId: string) => {
    ...
    updateAiSource(sourceId, (source) => {
      ...
      const remoteDescriptor = (modelsBySource[sourceId] || []).find((item) => item.id === draft);
      return {
        ...source,
        models: nextModels,
        modelsMeta: normalizeAiModelDescriptors([
          ...
          remoteDescriptor || { id: draft, capabilities: [selectedCapability] },
        ]),
  ...
  const fetchModelsForSource = useCallback(async (
    ...
    try {
      ...
      updateAiSource(source.id, (prev) => {
          ...
          const mergedModels = isOfficialManagedSource(source)
            ? normalizeSourceModels([...(prev.models || []), ...fetchedIds, prev.model])
            : normalizeSourceModels([...(prev.models || []), prev.model]);
          const fetchedById = new Map(deduped.map((item) => [item.id, item]));
          const mergedMeta = normalizeAiModelDescriptors([
            ...(prev.modelsMeta || []),
            ...mergedModels.map((id) => fetchedById.get(id) || { id }),
          ]);
  ...
  useEffect(() => {
    ...
    if (!baseURL || (!apiKey && !isLocalAiSource(activeAiSource))) return;
    const key = [baseURL, apiKey, activeAiSource.protocol || '', activeAiSource.presetId || ''].join('\n');
    if (autoFetchedModelsKeyRef.current[activeAiSource.id] === key) return;
    autoFetchedModelsKeyRef.current[activeAiSource.id] = key;
    const timer = window.setTimeout(() => {
      void fetchModelsForSource(activeAiSource);
    }, 450);
    return () => window.clearTimeout(timer);
  }, [activeAiSource, fetchModelsForSource, isLocalAiSource]);
  ...
  const handleOpenDownloadPage = useCallback(() => {
    void appAlert('Cohmira 正式下载页尚未配置，请等待发布仓库上线。');
  }, []);
      ...
      if (settings) {
        ...
        setFormData({
          ...
          image_model: (() => {
            ...
            return settings.image_model || DEFAULT_OPENAI_IMAGE_MODEL;
          })(),
          video_endpoint: settings.video_endpoint || DEFAULT_SEEDANCE_VIDEO_ENDPOINT,
          video_api_key: settings.video_api_key || '',
          video_model: settings.video_model || DEFAULT_SEEDANCE_VIDEO_MODEL,
  ...
  const loadSocialToolsStatus = useCallback(async () => {
    setIsSocialToolsLoading(true);
    try {
      const status = await window.ipcRenderer.socialTools.getStatus() as SocialToolsStatus;
      setSocialToolsStatus(status);
      if (status?.success && status.config) {
        setSocialToolsDraft(status.config);
        setSocialToolsMessage((current) => (
          current.startsWith('社媒工具状态读取失败') ? '' : current
        ));
      } else {
        setSocialToolsMessage(status?.error || '社媒工具状态读取失败');
      }
    } catch (error) {
      console.error('Failed to load social tools status', error);
      setSocialToolsMessage(`社媒工具状态读取失败：${String(error)}`);
    } finally {
      setIsSocialToolsLoading(false);
    }
  }, []);

  const handleSaveSocialTools = useCallback(async () => {
    if (!socialToolsDraft) return;
    setIsSavingSocialTools(true);
    try {
      const result = await window.ipcRenderer.socialTools.saveConfig(socialToolsDraft);
      if (!result?.success) {
        throw new Error(result?.error || '保存失败');
      }
      setSocialToolsMessage('社媒工具配置已保存。');
      const mcpResult = await window.ipcRenderer.mcp.list();
      if (mcpResult?.success && Array.isArray(mcpResult.servers)) {
        setMcpServers(mcpResult.servers as McpServerConfig[]);
      }
      await Promise.all([
        loadSocialToolsStatus(),
        loadMcpRuntimeData(),
      ]);
    } catch (error) {
      console.error('Failed to save social tools config', error);
      setSocialToolsMessage(`社媒工具配置保存失败：${String(error)}`);
    } finally {
      setIsSavingSocialTools(false);
    }
  }, [loadMcpRuntimeData, loadSocialToolsStatus, socialToolsDraft]);

  const handleCheckSocialToolAccount = useCallback(async (platform: SocialPublisherPlatform) => {
    const account = String(socialToolsDraft?.socialConnection.accounts[platform] || '').trim();
    setSocialToolsMessage(`正在检查 ${platform} 账号 ${account || 'default'}...`);
    try {
      if (socialToolsDraft) {
        await window.ipcRenderer.socialTools.saveConfig(socialToolsDraft);
      }
      const result = await window.ipcRenderer.socialTools.checkAccount({
        platform,
        account,
        proxyUrl: socialToolsDraft?.socialConnection.proxyUrl,
      });
      setSocialToolsMessage(result?.success
        ? `${platform} 账号 ${account || 'default'} 可用。`
        : `${platform} 账号 ${account || 'default'} 不可用：${result?.message || result?.error || result?.stderr || result?.stdout || '检查失败'}`);
      await loadSocialToolsStatus();
    } catch (error) {
      console.error('Failed to check social account', error);
      setSocialToolsMessage(`${platform} 账号检查失败：${String(error)}`);
    }
  }, [loadSocialToolsStatus, socialToolsDraft]);

  const handleImportSocialToolAccount = useCallback(async (
    platform: SocialPublisherPlatform,
    account: string,
    file: File,
  ) => {
    const profile = String(account || '').trim() || 'default';
    try {
      ...
      setSocialToolsMessage(`${platform} 账号 ${profile} 已导入。`);
      await loadSocialToolsStatus();
    } catch (error) {
      console.error('Failed to import social account', error);
      setSocialToolsMessage(`${platform} 账号导入失败：${String(error)}`);
    }
  }, [loadSocialToolsStatus]);

  const handleSetDefaultSocialToolAccount = useCallback(async (
    platform: SocialPublisherPlatform,
    account: string,
  ) => {
    if (!socialToolsDraft) return;
    ...
    const nextConfig: SocialToolsConfig = {
      ...socialToolsDraft,
      socialConnection: {
        ...socialToolsDraft.socialConnection,
        accounts: {
          ...socialToolsDraft.socialConnection.accounts,
          [platform]: profile,
        },
      },
    };
    setSocialToolsDraft(nextConfig);
    try {
      const result = await window.ipcRenderer.socialTools.saveConfig(nextConfig);
      if (!result?.success) {
        throw new Error(result?.error || '保存默认账号失败');
      }
      setSocialToolsMessage(`${platform} 默认账号已切换为 ${profile}。`);
      await loadSocialToolsStatus();
    } catch (error) {
      console.error('Failed to set default social account', error);
      setSocialToolsMessage(`${platform} 默认账号切换失败：${String(error)}`);
    }
  }, [loadSocialToolsStatus, socialToolsDraft]);

  const handleExportSocialToolAccount = useCallback(async (
    platform: SocialPublisherPlatform,
    account: string,
  ) => {
    const profile = String(account || '').trim() || 'default';
    try {
      const result = await window.ipcRenderer.socialTools.exportAccount({ platform, account: profile });
      ...
      anchor.href = url;
      anchor.download = String(result.filename || `${platform}_${profile}.json`);
      document.body.appendChild(anchor);
      anchor.click();
      anchor.remove();
      URL.revokeObjectURL(url);
      setSocialToolsMessage(`${platform} 账号 ${profile} 已导出。`);
    } catch (error) {
      console.error('Failed to export social account', error);
      setSocialToolsMessage(`${platform} 账号导出失败：${String(error)}`);
    }
  }, []);

  const handleDeleteSocialToolAccount = useCallback(async (
    platform: SocialPublisherPlatform,
    account: string,
  ) => {
    const profile = String(account || '').trim() || 'default';
    try {
      const result = await window.ipcRenderer.socialTools.deleteAccount({ platform, account: profile });
      if (!result?.success) {
        throw new Error(result?.error || result?.message || '删除失败');
      }
      setSocialToolsMessage(`${platform} 账号 ${profile} 已删除。`);
      await loadSocialToolsStatus();
    } catch (error) {
      console.error('Failed to delete social account', error);
      setSocialToolsMessage(`${platform} 账号删除失败：${String(error)}`);
    }
  }, [loadSocialToolsStatus]);

  const handleStartSocialToolLogin = useCallback(async (platform: SocialPublisherPlatform) => {
    const account = String(socialToolsDraft?.socialConnection.accounts[platform] || '').trim();
    const usesBrowserLogin = platform === 'youtube';
    setSocialToolLoginState({
      kind: 'social-connection',
      platform,
      account,
      running: true,
      message: `正在启动 ${platform} ${usesBrowserLogin ? '浏览器登录' : '扫码登录'}...`,
    });
    try {
      if (socialToolsDraft) {
        await window.ipcRenderer.socialTools.saveConfig(socialToolsDraft);
      ...
      const result = await window.ipcRenderer.socialTools.startLogin({
        platform,
        account,
        headless: socialToolsDraft?.socialConnection.headless,
        proxyUrl: socialToolsDraft?.socialConnection.proxyUrl,
        restart: true,
      });
      ...
      const stillRunning = Boolean(result?.running);
      setSocialToolLoginState({
        kind: 'social-connection',
        platform,
        account,
        ...
        success,
        message: success
          ? `${platform} 账号 ${account || 'default'} 登录完成。`
          : stillRunning
            ? `${platform} 账号 ${account || 'default'} 登录进程已启动：${result?.message || (usesBrowserLogin ? '请在浏览器完成登录' : '请扫码后检查账号状态')}`
            : `${platform} 账号 ${account || 'default'} 登录未完成：${result?.error || result?.message || stderr || stdout || '请查看输出'}`,
        qrcodeUrl: String(result?.qrcodeUrl || ''),
        qrcodePath: String(result?.qrcodePath || ''),
        status: String(result?.status || (success ? 'logged_in' : stillRunning ? 'pending' : 'failed')),
        stdout,
        stderr,
      ...
      setSocialToolsMessage(success
        ? '登录完成，账号状态已刷新。'
        : stillRunning
          ? usesBrowserLogin
            ? '请在弹出的浏览器完成登录，页面会自动更新并确认是否保存。'
            : '二维码生成后请扫码并在手机端确认；页面会自动更新并显示保存结果。'
          : '登录未完成，请查看登录状态输出。');
      await loadSocialToolsStatus();
    } catch (error) {
      ...
      setSocialToolLoginState({
        ...
        running: false,
        success: false,
        message: `${platform} 登录启动失败：${String(error)}`,
      });
    }
  }, [loadSocialToolsStatus, socialToolsDraft]);

  const handleStartMediaCrawlerLogin = useCallback(async () => {
    const platform = socialToolsDraft?.mediaCrawler.defaultPlatform || 'xhs';
    setSocialToolLoginState({
      kind: 'mediacrawler',
      mediaCrawlerPlatform: platform,
      running: true,
      message: `正在启动 MediaCrawler ${platform} 二维码登录任务...`,
      apiUrl: socialToolsDraft?.mediaCrawler.apiUrl,
    });
    try {
      if (socialToolsDraft) {
        await window.ipcRenderer.socialTools.saveConfig(socialToolsDraft);
      }
      const result = await window.ipcRenderer.socialTools.startMediaCrawlerLogin({
        platform,
        headless: false,
      });
      setSocialToolLoginState({
        kind: 'mediacrawler',
        mediaCrawlerPlatform: String(result?.platform || platform) as SocialToolLoginState['mediaCrawlerPlatform'],
        running: Boolean(result?.running),
        success: Boolean(result?.success),
        message: String(result?.message || result?.error || 'MediaCrawler 状态已更新。'),
        qrcodeUrl: String(result?.qrcodeUrl || ''),
        qrcodePath: String(result?.qrcodePath || ''),
        apiUrl: String(result?.apiUrl || socialToolsDraft?.mediaCrawler.apiUrl || ''),
        status: String(result?.status || ''),
        logs: Array.isArray(result?.logs) ? result.logs : [],
      });
      setSocialToolsMessage(String(result?.message || 'MediaCrawler 登录任务状态已更新。'));
      await loadSocialToolsStatus();
    } catch (error) {
      console.error('Failed to start MediaCrawler login', error);
      setSocialToolLoginState({
        kind: 'mediacrawler',
        mediaCrawlerPlatform: platform,
        running: false,
        success: false,
        message: `MediaCrawler 登录启动失败：${String(error)}`,
        apiUrl: socialToolsDraft?.mediaCrawler.apiUrl,
      });
    }
  }, [loadSocialToolsStatus, socialToolsDraft]);

  const handleRefreshSocialToolLoginStatus = useCallback(async () => {
    const state = socialToolLoginState;
    if (state.kind === 'social-connection' && state.platform) {
      ...
      setSocialToolLoginState({
        kind: 'social-connection',
        platform: state.platform,
        account: String(result?.account || state.account || ''),
        running: Boolean(result?.running),
        success: Boolean(result?.success),
        message: String(result?.message || result?.error || '账号登录状态已刷新。'),
        qrcodeUrl: String(result?.qrcodeUrl || ''),
        qrcodePath: String(result?.qrcodePath || ''),
        status: String(result?.status || ''),
        stdout,
    ...
    if (state.kind === 'mediacrawler') {
      const result = await window.ipcRenderer.socialTools.getMediaCrawlerStatus();
      setSocialToolLoginState({
        kind: 'mediacrawler',
        mediaCrawlerPlatform: String(result?.platform || state.mediaCrawlerPlatform || 'xhs') as SocialToolLoginState['mediaCrawlerPlatform'],
        running: Boolean(result?.running),
        success: Boolean(result?.success),
        message: String(result?.message || result?.error || 'MediaCrawler 状态已刷新。'),
        qrcodeUrl: String(result?.qrcodeUrl || ''),
        qrcodePath: String(result?.qrcodePath || ''),
        apiUrl: String(result?.apiUrl || state.apiUrl || ''),
        status: String(result?.status || ''),
        logs: Array.isArray(result?.logs) ? result.logs : [],
      });
      await loadSocialToolsStatus();
    }
  }, [loadSocialToolsStatus, socialToolLoginState]);

  useEffect(() => {
    if (!socialToolLoginState.running || !socialToolLoginState.kind) return undefined;
    let disposed = false;
    const poll = async () => {
      if (disposed || socialToolLoginPollInFlightRef.current) return;
      socialToolLoginPollInFlightRef.current = true;
      try {
        await handleRefreshSocialToolLoginStatus();
      } catch (error) {
        if (!disposed) {
          console.error('Failed to auto-refresh social login status', error);
        }
      } finally {
        socialToolLoginPollInFlightRef.current = false;
      }
    };
    ...
    socialToolLoginState.account,
    socialToolLoginState.kind,
    socialToolLoginState.platform,
    socialToolLoginState.running,
  ]);

  const handleStopSocialToolLogin = useCallback(async () => {
    const state = socialToolLoginState;
    try {
      if (state.kind === 'social-connection' && state.platform) {
        const result = await window.ipcRenderer.socialTools.stopLogin({
          platform: state.platform,
          account: state.account,
        });
        setSocialToolLoginState((current) => ({
          ...current,
          running: false,
          success: String(result?.status || '') === 'logged_in',
          status: String(result?.status || 'stopped'),
          message: String(result?.message || result?.error || '登录进程已停止。'),
          qrcodeUrl: String(result?.qrcodeUrl || ''),
          qrcodePath: String(result?.qrcodePath || ''),
          stdout: String(result?.stdout || current.stdout || ''),
          stderr: String(result?.stderr || current.stderr || ''),
          accountFile: String(result?.accountFile || current.accountFile || ''),
          accountExists: Boolean(result?.accountExists),
          accountUpdatedAt: typeof result?.accountUpdatedAt === 'number' ? result.accountUpdatedAt : current.accountUpdatedAt,
        }));
        await loadSocialToolsStatus();
      } else if (state.kind === 'mediacrawler') {
        const result = await window.ipcRenderer.socialTools.stopMediaCrawler();
        setSocialToolLoginState((current) => ({
          ...current,
          running: Boolean(result?.running),
          success: Boolean(result?.success),
          message: String(result?.message || result?.error || 'MediaCrawler 任务已停止。'),
          status: String(result?.status || ''),
  ...
  const handleCloseSocialToolLoginDialog = useCallback(() => {
    setSocialToolLoginState({
      running: false,
      message: '',
    });
  ...
  useEffect(() => {
    if (!isActive || activeTab !== 'tools' || !socialToolLoginState.kind || !socialToolLoginState.running) return;
    const timer = window.setInterval(() => {
      void handleRefreshSocialToolLoginStatus();
    }, 2500);
  ...
  const handleOpenSocialCookiesDir = useCallback(async () => {
    try {
      const result = await window.ipcRenderer.socialTools.openSocialCookiesDir();
      if (!result?.success) {
        throw new Error(result?.error || '打开账号目录失败');
      }
      setSocialToolsMessage(`已打开账号目录：${result.path || ''}`);
    } catch (error) {
      setSocialToolsMessage(`打开账号目录失败：${String(error)}`);
    }
  }, []);

  const handleOpenMediaCrawlerBrowserDataDir = useCallback(async () => {
    try {
      const result = await window.ipcRenderer.socialTools.openMediaCrawlerBrowserDataDir();
      if (!result?.success) {
        throw new Error(result?.error || '打开登录态目录失败');
      }
      setSocialToolsMessage(`已打开 MediaCrawler 登录态目录：${result.path || ''}`);
    } catch (error) {
      setSocialToolsMessage(`打开 MediaCrawler 登录态目录失败：${String(error)}`);
    }
  }, []);
  ...
  const handlePrepareBrowserPlugin = async () => {
    ...
    try {
      ...
      void appAlert(`插件已同步到最新内置版本。\n\n外层目录：${result.path}\n插件目录：${result.pluginPath || '未返回'}\n\n下一步请打开 Chrome / Edge 扩展管理页，开启开发者模式后，将里面的“商媒运营助手 Browser Extension”文件夹拖进浏览器，或在“加载已解压的扩展程序”里选择该插件文件夹。`);
    } catch (error) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const resolvedVideoModel = String(formData.video_model || '').trim() || DEFAULT_SEEDANCE_VIDEO_MODEL;
      ...
      await window.ipcRenderer.saveSettings({
        ...
        video_endpoint: String(formData.video_endpoint || DEFAULT_SEEDANCE_VIDEO_ENDPOINT).trim(),
  ...
  const persistRedClawPromptPresets = useCallback((nextPresets: RedClawPromptPreset[]) => {
    setRedClawPromptPresets(saveRedClawPromptPresets(nextPresets));
  }, []);

  const updateRedClawPromptPreset = useCallback((presetId: string, patch: Partial<RedClawPromptPreset>) => {
    persistRedClawPromptPresets(redClawPromptPresets.map((preset) => (
      preset.id === presetId ? { ...preset, ...patch } : preset
    )));
  }, [persistRedClawPromptPresets, redClawPromptPresets]);

  const addRedClawPromptPreset = useCallback(() => {
    persistRedClawPromptPresets([...redClawPromptPresets, createRedClawPromptPreset()]);
  }, [persistRedClawPromptPresets, redClawPromptPresets]);

  const removeRedClawPromptPreset = useCallback((presetId: string) => {
    persistRedClawPromptPresets(redClawPromptPresets.filter((preset) => preset.id !== presetId));
  }, [persistRedClawPromptPresets, redClawPromptPresets]);

  const resetRedClawPromptPresets = useCallback(() => {
    persistRedClawPromptPresets(DEFAULT_REDCLAW_PROMPT_PRESETS);
  }, [persistRedClawPromptPresets]);
          ...
          <form onSubmit={handleSave} className="space-y-10">
                ...
                <section className="space-y-6">
                  ...
                  <div className="rounded-xl border border-border bg-surface-secondary/20 p-3 space-y-3">
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        <h3 className="text-sm font-medium text-text-primary">创作预设 Prompt</h3>
                        <p className="mt-1 text-[11px] text-text-tertiary">
                          点击图片、视频等创作入口时，会先放入输入框的预设段。
                        </p>
                      </div>
                      <div className="flex items-center gap-2">
                        <button
                          type="button"
                          onClick={resetRedClawPromptPresets}
                          className="px-2.5 py-1.5 text-[11px] border border-border rounded hover:bg-surface-secondary transition-colors"
                        >
                          恢复默认
                        </button>
                        <button
                          type="button"
                          onClick={addRedClawPromptPreset}
                          className="flex items-center gap-1.5 px-2.5 py-1.5 text-[11px] border border-border rounded hover:bg-surface-secondary transition-colors"
                        >
                    ...
                    <div className="space-y-2">
                      {redClawPromptPresets.map((preset) => (
                        <div key={preset.id} className="rounded-lg border border-border bg-surface-primary/70 p-2.5 space-y-2">
                          <div className="flex items-center gap-2">
                            <input
                              type="text"
                              value={preset.label}
                              onChange={(event) => updateRedClawPromptPreset(preset.id, { label: event.target.value })}
                              className="min-w-0 flex-1 rounded border border-border bg-surface-secondary/30 px-2.5 py-1.5 text-xs text-text-primary focus:outline-none focus:border-accent-primary"
                              placeholder="预设名称"
                              aria-label={`${preset.label || '未命名'}预设名称`}
                            />
                            <button
                              type="button"
                              onClick={() => removeRedClawPromptPreset(preset.id)}
                              className="rounded p-1.5 text-text-tertiary hover:bg-red-500/10 hover:text-red-500 transition-colors"
                              title="删除预设"
                            >
                          ...
                          <textarea
                            value={preset.text}
                            onChange={(event) => updateRedClawPromptPreset(preset.id, { text: event.target.value })}
                            className="min-h-[76px] w-full resize-y rounded border border-border bg-surface-secondary/30 px-2.5 py-2 text-xs leading-5 text-text-primary focus:outline-none focus:border-accent-primary"
                            placeholder="预设 Prompt 内容"
                            aria-label={`${preset.label || '未命名'}预设 Prompt 内容`}
                          />
                        </div>
                      ))}
                    </div>
                      ...
                      {showScopedModelOverrides && (
                        <div className="px-3 pb-3 space-y-3 border-t border-border/70">
                          <p className="text-[11px] text-text-tertiary pt-3">
                            留空表示跟随“默认聊天模型”。此配置面向高级用户，分别作用于漫步、群聊、知识库、商媒运营助手。
                          </p>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            ...
                            <div className="space-y-1">
                              <label className="text-[11px] text-text-secondary">商媒运营助手默认模型</label>
                              <input
                                type="text"
                                list="scoped-model-candidates"
                      ...
                      <div className="group">
                        ...
                        <AiModelSelect
                          ...
                          options={transcriptionSourceModels.map((model) => {
                            ...
                            const isRecommended = normalizedModelId === 'doubao-seed-asr-2.0' || normalizedModelId === 'step-asr';
                            return {
                              id: model.id,
                              label: model.id,
                  ...
                  <div className="pt-4 border-t border-border">
                    ...
                    <div className="rounded-xl border border-border bg-surface-secondary/20 p-3 space-y-3">
                      <div className="rounded-lg border border-border bg-surface-primary/70 p-3 text-xs text-text-secondary">
                        默认使用 Ark Plan Seedance 视频接口；商媒运营助手兼容模型仍可在需要时手动填写。
                      </div>
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                        <div className="rounded-lg border border-border bg-surface-primary/70 p-3">
                          <div className="text-[11px] text-text-tertiary mb-1">默认视频 Endpoint</div>
                          <div className="text-sm font-medium text-text-primary break-all">{DEFAULT_SEEDANCE_VIDEO_ENDPOINT}</div>
                        </div>
                        <div className="rounded-lg border border-border bg-surface-primary/70 p-3">
                          <div className="text-[11px] text-text-tertiary mb-1">默认视频模型</div>
                          <div className="text-sm font-medium text-text-primary">{DEFAULT_SEEDANCE_VIDEO_MODEL}</div>
                        </div>
                        <div className="rounded-lg border border-border bg-surface-primary/70 p-3">
                          <div className="text-[11px] text-text-tertiary mb-1">商媒运营助手兼容模型</div>
                          <div className="text-sm font-medium text-text-primary">{REDBOX_OFFICIAL_VIDEO_MODEL_LIST.join(' / ')}</div>
                        </div>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      ...
                      <div className="group">
                        ...
                        <input
                          ...
                          aria-label="DeepSeek max_tokens"
                  ...
                  <div className="pt-4 border-t border-border">
                    <h3 className="text-sm font-medium text-text-primary mb-4">商媒运营助手上下文压缩策略</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      <div className="group">
                        <label className="block text-xs font-medium text-text-secondary mb-1.5">
                          自动压缩阈值（tokens）
                        ...
                        />
                        <p className="mt-1 text-[11px] text-text-tertiary">
                          默认 256000。商媒运营助手对话预计上下文超过该值时会自动 compact。
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between gap-3">
                    <div className="min-w-0">
                      <div className="flex items-center gap-3">
                        ...
                        >
                          {redclawOnboardingCompleted ? '重新调整偏好' : '设置协作偏好'}
                        </button>
                      </div>
            ...
            {activeTab === 'tools' && (
              <ToolsSettingsSection
                ...
                socialToolsDraft={socialToolsDraft}
                setSocialToolsDraft={setSocialToolsDraft}
                socialToolsMessage={socialToolsMessage}
                isSocialToolsLoading={isSocialToolsLoading}
                isSavingSocialTools={isSavingSocialTools}
                socialToolLoginState={socialToolLoginState}
                handleRefreshSocialTools={loadSocialToolsStatus}
                handleSaveSocialTools={handleSaveSocialTools}
                handleCheckSocialToolAccount={handleCheckSocialToolAccount}
                handleStartSocialToolLogin={handleStartSocialToolLogin}
                handleImportSocialToolAccount={handleImportSocialToolAccount}
                handleSetDefaultSocialToolAccount={handleSetDefaultSocialToolAccount}
                handleExportSocialToolAccount={handleExportSocialToolAccount}
                handleDeleteSocialToolAccount={handleDeleteSocialToolAccount}
                handleRefreshSocialToolLoginStatus={handleRefreshSocialToolLoginStatus}
                handleStopSocialToolLogin={handleStopSocialToolLogin}
                handleStartMediaCrawlerLogin={handleStartMediaCrawlerLogin}
                handleCloseSocialToolLoginDialog={handleCloseSocialToolLoginDialog}
                handleOpenSocialCookiesDir={handleOpenSocialCookiesDir}
                handleOpenMediaCrawlerBrowserDataDir={handleOpenMediaCrawlerBrowserDataDir}
