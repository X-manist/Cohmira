...
const REDBOX_OFFICIAL_LOGO_URL = '/cohmira-mark.svg';
...
type AiModelDescriptorInput = {
  ...
  context_limit?: number | string | null;
...
const positiveModelLimit = (...values: unknown[]): number | undefined => {
...
const normalizeModalities = (value: unknown): string[] => Array.isArray(value)
...
export const normalizeAiModelDescriptors = (
  models: Array<
    ...
    | null
    | undefined
    | AiModelDescriptorInput
  >,
): AiModelDescriptor[] => {
  const merged = new Map<string, AiModelDescriptor>();
  for (const raw of models) {
    const descriptor = toAiModelDescriptor(raw as string | AiModelDescriptorInput);
    ...
    merged.set(descriptor.id, {
      ...
      contextLimit: descriptor.contextLimit ?? previous?.contextLimit,
      maxOutputTokens: descriptor.maxOutputTokens ?? previous?.maxOutputTokens,
      reasoning: descriptor.reasoning ?? previous?.reasoning,
      supportsTools: descriptor.supportsTools ?? previous?.supportsTools,
      inputModalities: Array.from(new Set([...(previous?.inputModalities || []), ...(descriptor.inputModalities || [])])),
      outputModalities: Array.from(new Set([...(previous?.outputModalities || []), ...(descriptor.outputModalities || [])])),
    });
  }
...
export const toAiModelDescriptor = (
  model: string | AiModelDescriptorInput,
  ...
  return {
    ...
    inputCapabilities: getModelInputCapabilities(id),
    contextLimit: positiveModelLimit(model.contextLimit, model.context_limit, model.contextLength, model.context_length),
    maxOutputTokens: positiveModelLimit(model.maxOutputTokens, model.max_output_tokens),
    reasoning: typeof model.reasoning === 'boolean' ? model.reasoning : undefined,
    supportsTools: typeof model.supportsTools === 'boolean'
      ? model.supportsTools
      : (typeof model.supports_tools === 'boolean' ? model.supports_tools : undefined),
  ...
  return (
    <div className="relative">
      ...
      <button
        ...
        tabIndex={-1}
        aria-label={show ? '隐藏敏感信息' : '显示敏感信息'}
        title={show ? '隐藏敏感信息' : '显示敏感信息'}
      >
