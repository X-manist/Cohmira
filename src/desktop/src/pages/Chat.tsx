...
import { loadAttachmentDrafts, saveAttachmentDrafts } from '../features/chat/attachmentDraftStore';
...
const BUILTIN_SLASH_COMMANDS: ChatComposerSlashCommand[] = [
  { name: 'help', description: '查看所有内置、配方和技能命令' },
  { name: 'skills', description: '列出当前可用的 Skills' },
  { name: 'goal', description: '设置持续目标；使用 /goal off 清除' },
  { name: 'loop', description: '持续执行目标直到完成或达到最大轮次；使用 /loop off 清除' },
  { name: 'compact', description: '压缩当前会话上下文' },
  { name: 'clear', description: '清空当前 Goose 会话上下文' },
  { name: 'doctor', description: '检查 Goose 与扩展运行状态' },
  { name: 'prompts', description: '列出 MCP 扩展提供的 Prompts' },
  { name: 'prompt', description: '运行指定 Prompt' },
  { name: 'grind', description: '持续目标模式（/loop 的原始命令）' },
];
...
interface ChatProps {
  ...
  welcomeActions?: ChatWelcomeAction[];
...
interface ChatRuntimeState {
  ...
  updatedAt: number;
  runtimeMode?: string;
  runtimeEvents?: RuntimeUnifiedEvent[];
}

interface ForkSendDraft {
  sourceSessionId: string;
  sourceMessageId: string;
}

interface SendMessageOptions {
  forkDraft?: ForkSendDraft;
}
...
function stripTransientAttachmentPreviews(
  attachments: UploadedFileAttachment[],
): UploadedFileAttachment[] {
  return attachments
    .map((attachment) => stripTransientAttachmentPreview(attachment))
...
function applyAttachmentDeliveryModes(
  attachments: UploadedFileAttachment[],
  modelName?: string,
): UploadedFileAttachment[] {
  return attachments
...
function finishRunningTimelineItems(timeline: ProcessItem[] = [], now = Date.now()): ProcessItem[] {
  return timeline.map((item) => (
    item.status === 'running'
      ? { ...item, status: 'done', duration: now - item.timestamp } as ProcessItem
      : item
  ));
}
...
function createAssistantSegment(
  ...
  return {
    id: `streaming_${now}_${Math.random().toString(36).slice(2, 8)}`,
    role: 'ai',
    messageType: 'reply',
    content: '',
    tools: [],
...
function buildToolTimelineItem(toolData: { callId: string; name: string; input: unknown; description?: string }, now = Date.now()): ProcessItem {
  return {
    id: `tool_${toolData.callId || now}_${Math.random().toString(36).slice(2, 8)}`,
    type: 'tool-call',
...
function buildToolEvent(toolData: { callId: string; name: string; input: unknown; description?: string }): ToolEvent {
  return {
    id: `tool_${toolData.callId || Date.now()}_${Math.random().toString(36).slice(2, 8)}`,
    callId: toolData.callId,
    name: toolData.name,
    input: toolData.input,
...
function appendToolStartSegment(
  messages: Message[],
  toolData: { callId: string; name: string; input: unknown; description?: string },
  timestamp = Date.now(),
): Message[] {
  const runningThinkingIndex = findLastRunningThinkingIndex(messages);
  ...
  const timelineItem = buildToolTimelineItem(toolData, timestamp);
  const toolEvent = buildToolEvent(toolData);

  if (lastReplyIndex === -1) {
    next.push(createAssistantSegment(timestamp, {
  ...
  const hasPriorResponseText = String(lastMsg.content || '').trim().length > 0;
  if (hasPriorResponseText) {
    next[lastReplyIndex] = {
      ...lastMsg,
      timeline: finishRunningTimelineItems(lastMsg.timeline || [], timestamp),
      isStreaming: false,
    ...
    next.push(createAssistantSegment(timestamp, {
      timeline: [timelineItem],
      tools: [toolEvent],
      processingStartedAt: lastMsg.processingStartedAt ?? timestamp,
    }));
    return next;
  }
  ...
  next[lastReplyIndex] = {
    ...lastMsg,
    messageType: 'reply',
    timeline: [...(lastMsg.timeline || []), timelineItem],
    tools: [...(lastMsg.tools || []), toolEvent],
    isStreaming: true,
    suppressPendingIndicator: true,
    processingStartedAt: lastMsg.processingStartedAt ?? timestamp,
  };
...
function appendResponseDeltaSegment(
  messages: Message[],
  content: string,
  timestamp = Date.now(),
  ...
  if (lastReplyIndex === -1 || !next[lastReplyIndex].isStreaming) {
    next.push(createAssistantSegment(timestamp, {
      content,
      suppressPendingIndicator: false,
      processingStartedAt: processingStartedAt ?? timestamp,
    }));
    return next;
  }
  const lastMsg = next[lastReplyIndex];
  next[lastReplyIndex] = {
    ...
    messageType: 'reply',
    suppressPendingIndicator: false,
    processingStartedAt: lastMsg.processingStartedAt ?? processingStartedAt ?? timestamp,
  };
  return next;
}
...
function updateToolSegment(
  ...
  toolData: {
    callId: string;
    name: string;
    output: NonNullable<ToolEvent['output']> & { partial?: boolean };
  },
  timestamp = Date.now(),
): Message[] {
  const next = [...messages];
  for (let messageIndex = next.length - 1; messageIndex >= 0; messageIndex -= 1) {
    ...
    if (!isAssistantReplyMessage(message)) continue;
    const timeline = [...(message.timeline || [])];
    let matchedIndex = findLatestTimelineItemIndex(
      timeline,
      (item) => item.type === 'tool-call' && item.toolData?.callId === toolData.callId,
    );
    if (matchedIndex === -1) {
      matchedIndex = findLatestTimelineItemIndex(
        timeline,
        (item) => item.type === 'tool-call' && item.status === 'running' && item.toolData?.name === toolData.name,
      );
    }
    if (matchedIndex === -1) continue;

    const targetItem = timeline[matchedIndex];
    const content = String(toolData.output?.content || '');
    const normalizedOutput = {
      ...toolData.output,
      success: toolData.output?.success === undefined ? true : Boolean(toolData.output.success),
      content,
    };
    if (toolData.output?.partial) {
      const currentOutput = typeof targetItem.toolData?.output === 'string'
        ? targetItem.toolData.output
        : String((targetItem.toolData?.output as Record<string, unknown> | undefined)?.content || '');
      let mergedOutput = currentOutput;
      if (!currentOutput) {
        mergedOutput = content;
      } else if (content.startsWith(currentOutput)) {
        mergedOutput = content;
      } else if (!currentOutput.endsWith(content)) {
        mergedOutput = `${currentOutput}\n${content}`;
      }
      timeline[matchedIndex] = {
        ...targetItem,
        toolData: {
          ...targetItem.toolData!,
          output: mergedOutput,
      ...
      timeline[matchedIndex] = {
        ...
        duration: timestamp - targetItem.timestamp,
        toolData: {
          ...targetItem.toolData!,
    ...
    const tools = (message.tools || []).map((tool) => (
      tool.callId === toolData.callId
        ? {
            ...tool,
            status: toolData.output?.partial ? tool.status : (normalizedOutput.success ? 'done' : 'failed'),
            output: normalizedOutput,
          } as ToolEvent
        : tool
    ));
...
function runtimePayload(event: RuntimeUnifiedEvent): Record<string, unknown> {
...
function replayRuntimeEventsIntoMessages(
  baseMessages: Message[],
  events: RuntimeUnifiedEvent[],
  processingStartedAt?: number,
): Message[] {
  ...
  for (const event of events) {
    ...
    if (event.eventType === 'runtime:stream-start') {
      const phase = String(payload.phase || '').trim();
      if (!phase) continue;
      const lastReplyIndex = findLastAssistantReplyIndex(next);
      const phaseItem: ProcessItem = {
        id: `phase_${timestamp}_${Math.random().toString(36).slice(2, 8)}`,
        type: 'phase',
        title: phase,
        content: '',
        status: 'running',
      ...
      if (lastReplyIndex === -1 || !next[lastReplyIndex].isStreaming) {
        ...
        next[lastReplyIndex] = {
          ...lastMsg,
          timeline: [...(lastMsg.timeline || []), phaseItem],
          isStreaming: true,
          processingStartedAt: lastMsg.processingStartedAt ?? processingStartedAt ?? timestamp,
        };
      }
    ...
    if (event.eventType === 'runtime:text-delta') {
      const content = String(payload.content || '');
      if (!content) continue;
      if (String(payload.stream || 'response') === 'thought') {
        const lastReplyIndex = findLastAssistantReplyIndex(next);
        const thoughtItem: ProcessItem = {
          id: `thought_${timestamp}_${Math.random().toString(36).slice(2, 8)}`,
          type: 'thought',
          title: '思考过程',
          content,
          status: 'running',
        ...
        if (lastReplyIndex === -1 || !next[lastReplyIndex].isStreaming) {
          ...
          next[lastReplyIndex] = {
            ...lastMsg,
            timeline: [...(lastMsg.timeline || []), thoughtItem],
            isStreaming: true,
          };
        }
        continue;
      }
      next = appendResponseDeltaSegment(next, content, timestamp, processingStartedAt);
      continue;
    }

    if (event.eventType === 'runtime:tool-start') {
      next = appendToolStartSegment(next, {
    ...
    if (event.eventType === 'runtime:tool-update' || event.eventType === 'runtime:tool-end') {
      const output = runtimePayload({ ...event, payload: payload.output } as RuntimeUnifiedEvent);
      next = updateToolSegment(next, {
        callId: String(payload.callId || ''),
        name: String(payload.name || 'tool'),
        output: {
          ...output,
          success: output.success === undefined ? true : Boolean(output.success),
          content: String(output.content || ''),
          partial: event.eventType === 'runtime:tool-update' || Boolean(output.partial),
        },
      }, timestamp);
      continue;
    }

    if (event.eventType === 'runtime:done') {
      const lastReplyIndex = findLastAssistantReplyIndex(next);
      if (lastReplyIndex !== -1) {
        const lastMsg = next[lastReplyIndex];
        next[lastReplyIndex] = {
...
function restorePersistedToolCalls(
  value: unknown,
  fallbackTimestamp = Date.now(),
): { tools: ToolEvent[]; timeline: ProcessItem[] } {
  let parsed = value;
  if (typeof parsed === 'string') {
    const text = parsed.trim();
    if (!text) return { tools: [], timeline: [] };
    try {
      parsed = JSON.parse(text);
    } catch {
      return { tools: [], timeline: [] };
    }
  }
  if (!Array.isArray(parsed)) return { tools: [], timeline: [] };

  const tools: ToolEvent[] = [];
  const timeline: ProcessItem[] = [];
  parsed.forEach((item, index) => {
    if (!item || typeof item !== 'object') return;
    const record = item as Record<string, unknown>;
    const callId = String(record.callId || record.call_id || record.id || `history-tool-${index}`).trim();
    const name = String(record.name || record.toolName || record.tool_name || 'tool').trim() || 'tool';
    const rawStatus = String(record.status || '').trim().toLowerCase();
    const outputRecord = record.output && typeof record.output === 'object' && !Array.isArray(record.output)
      ? record.output as Record<string, unknown>
      : {};
    const success = outputRecord.success === undefined
      ? rawStatus !== 'failed'
      : Boolean(outputRecord.success);
    const status: ToolEvent['status'] = rawStatus === 'running'
      ? 'running'
      : (success ? 'done' : 'failed');
    const outputContent = typeof record.output === 'string'
      ? record.output
      : String(outputRecord.content || '');
    const normalizedOutput = record.output && typeof record.output === 'object' && !Array.isArray(record.output)
      ? {
          ...outputRecord,
          success,
          content: outputContent,
        } as NonNullable<ToolEvent['output']>
      : outputContent
        ? { success, content: outputContent } as NonNullable<ToolEvent['output']>
        : undefined;
    const startedAt = parseMessageTimestampMs(record.startedAt || record.started_at) || fallbackTimestamp;
    const finishedAt = parseMessageTimestampMs(record.finishedAt || record.finished_at);
    const description = String(record.description || '').trim();
    const input = record.input ?? record.arguments ?? {};

    tools.push({
      id: `history-${callId}-${index}`,
      callId,
      name,
      input,
      output: normalizedOutput,
    ...
    timeline.push({
      id: `history-timeline-${callId}-${index}`,
      type: 'tool-call',
      title: description || undefined,
      content: description,
      status,
      timestamp: startedAt,
      duration: finishedAt ? Math.max(0, finishedAt - startedAt) : undefined,
      toolData: {
        callId,
        name,
        input,
...
const PROVIDER_PROTOCOL_DUMP_RE = /(?:response\.completed|sequence_number|input_tokens_details|output_tokens_details|[\\"]tools[\\"]\s*:|[\\"]usage[\\"]\s*:)/i;

function sanitizeChatErrorSummary(value: unknown, fallback: string): string {
  const raw = String(value || '').trim();
  if (!raw) return fallback;
  if (raw.includes('Failed to parse Responses stream event') || raw.includes('Stream decode error')) {
    return '模型响应格式暂不兼容';
  }
  const firstLine = raw.split('\n')[0]
    .split('Raw response:')[0]
    .trim();
  if (!firstLine) return fallback;
  return firstLine.length > 240 ? `${firstLine.slice(0, 240)}...` : firstLine;
}

function sanitizeChatErrorDetail(value: unknown): string {
  const raw = String(value || '').trim();
  if (!raw || PROVIDER_PROTOCOL_DUMP_RE.test(raw)) return '';
  if (raw.includes('Failed to parse Responses stream event') || raw.includes('Stream decode error')) {
    return '';
  }
  const withoutRawResponse = raw.split('\nRaw response:')[0].trim();
  return withoutRawResponse.length > 1_200
    ? `${withoutRawResponse.slice(0, 1_200)}...`
    : withoutRawResponse;
}
...
function normalizeChatErrorNotice(payload: ChatErrorEventPayload | string | null | undefined): StructuredChatErrorNotice {
  ...
  const title = sanitizeChatErrorSummary(data.title || data.message, '请求失败');
  const detail = sanitizeChatErrorDetail(data.detail || data.raw);
  const hint = sanitizeChatErrorSummary(data.hint, '');
...
export function Chat({
  ...
  const [composerPresetPrompt, setComposerPresetPrompt] = useState<{ label: string; text: string } | null>(null);
  ...
  const [slashCommands, setSlashCommands] = useState<ChatComposerSlashCommand[]>(BUILTIN_SLASH_COMMANDS);
  ...
  useEffect(() => {
    let disposed = false;
    void window.ipcRenderer.listSkills()
      .then((skills) => {
        if (disposed || !Array.isArray(skills)) return;
        const seen = new Set(BUILTIN_SLASH_COMMANDS.map((command) => command.name.toLocaleLowerCase()));
        const skillCommands = skills
          .filter((skill) => !skill.disabled && String(skill.name || '').trim())
          .map((skill) => ({
            name: String(skill.name).replace(/^\//, '').trim(),
            description: String(skill.description || '运行此 Skill').trim(),
          }))
          .filter((command) => {
        ...
        setSlashCommands([...BUILTIN_SLASH_COMMANDS, ...skillCommands]);
      })
      .catch(() => {
        // 内置命令仍可用；Skills 列表会在后续页面刷新时重新加载。
      });
    return () => {
      disposed = true;
    };
  ...
  const removePendingAttachment = useCallback((_attachment: UploadedFileAttachment, index: number) => {
  ...
  const dispatchChatSend = useCallback((payload: {
    ...
    debugUi('dispatch_send:queued', {
      ...
      hasAttachment: Boolean(payload.attachment || payload.attachments?.length),
    });
  ...
  useEffect(() => {
    ...
    const sendPendingMessage = async () => {
      ...
      const userMsg: Message = {
        ...
        displayContent: pendingMessage.displayContent || pendingMessage.content,
      ...
      const aiPlaceholder: Message = {
        ...
        content: '',
        tools: [],
        timeline: buildPendingAssistantTimeline(
          pendingMessage.taskHints?.forceMultiAgent ? '任务已提交' : '正在思考',
        ),
        isStreaming: true,
        processingStartedAt,
      };
  ...
  const loadSessions = async () => {
    ...
    try {
      ...
      if (
        ...
        && loadAttachmentDrafts('chat', '__new__').length === 0
      ) {
  ...
  const selectSession = async (sessionId: string) => {
    ...
    try {
      const wasColdRecoveryPending = coldRecoveryPendingRef.current;
      const shouldRecoverRuntime = true;
      coldRecoveryPendingRef.current = false;
      debugUi('select_session:start', { sessionId, shouldRecoverRuntime, wasColdRecoveryPending });
      ...
      let uiMessages: Message[] = history.map((msg: any) => {
        ...
        let attachments: UploadedFileAttachment[] | undefined;
        if (msg.attachment) {
          try {
            const parsed = typeof msg.attachment === 'string' ? JSON.parse(msg.attachment) : msg.attachment;
            if (Array.isArray(parsed)) {
              attachments = parsed.filter((item): item is UploadedFileAttachment => item?.type === 'uploaded-file');
              attachment = (attachments[0] || parsed[0]) as Message['attachment'] | undefined;
            } else {
              attachment = parsed as Message['attachment'];
            }
          } catch (e) {
        ...
        const restoredTools = role === 'ai'
          ? restorePersistedToolCalls(msg.toolCalls ?? msg.tool_calls, createdAt)
          : { tools: [], timeline: [] };
        ...
        return {
          ...
          timeline: restoredTools.timeline,
          isStreaming: false,
          processingStartedAt,
      ...
      // 每次切回会话都读取对应 runtimeState；非当前会话的事件由后端持续落库，
      // 因此可从已保存草稿恢复，而不会把另一个会话的流误接到当前 UI。
      if (shouldRecoverRuntime && runtimeProcessing) {
        debugUi('cold_recovery:runtime_processing', {
          sessionId,
          partialChars: runtimePartial.length,
          runtimeEvents: Array.isArray(runtimeState?.runtimeEvents) ? runtimeState.runtimeEvents.length : 0,
        });
        const runtimeEvents = Array.isArray(runtimeState?.runtimeEvents)
          ? runtimeState.runtimeEvents
          : [];
        if (runtimeEvents.length > 0) {
          uiMessages = replayRuntimeEventsIntoMessages(
            uiMessages,
            runtimeEvents,
            lastUserCreatedAt ?? Date.now(),
          ).map((message, index, all) => (
            index === findLastAssistantReplyIndex(all)
              ? { ...message, isStreaming: true, processingFinishedAt: undefined }
              : message
          ));
        } else {
  ...
  const appendAssistantChunk = useCallback((chunk: string) => {
    ...
    setMessages(prev => {
      ...
      return appendResponseDeltaSegment(prev, chunk, Date.now());
    });
  }, []);
  ...
  const handleResponseChunk = (_: unknown, { content }: { content: string }) => {
    ...
    const handleToolStart = (_: unknown, toolData: { callId: string; name: string; input: unknown; description?: string }) => {
      if (!isActiveRef.current) return;
      setMessages(prev => appendToolStartSegment(prev, toolData));
    };

    const handleToolEnd = (_: unknown, toolData: {
      callId: string;
      name: string;
      output: NonNullable<ToolEvent['output']>;
    }) => {
      if (!isActiveRef.current) return;
      setMessages(prev => updateToolSegment(prev, toolData));
    };

    const handleToolUpdate = (_: unknown, toolData: { callId: string; name: string; partial: string }) => {
      if (!isActiveRef.current) return;
      if (!toolData?.partial) return;
      setMessages(prev => updateToolSegment(prev, {
        callId: toolData.callId,
        name: toolData.name,
        output: {
          success: true,
          content: toolData.partial,
          partial: true,
        },
      }));
    };
    ...
    const disposeRuntimeEvents = subscribeRuntimeEventStream({
      ...
      onChatDone: ({ status, content, reason }) => {
        ...
        if (status === 'failed' && !responseCompletedRef.current) {
          handleError(null, {
            message: 'Agent 运行失败',
            raw: content || reason || 'runtime failed',
            hint: content || '请检查 Goose runtime、模型配置和 API Key。',
          });
        }
      },
  ...
  const appendPendingAttachments = useCallback((attachments: UploadedFileAttachment[]) => {
  ...
  const pickAttachment = useCallback(async () => {
    ...
    try {
      ...
      const attachments = result.attachments?.filter(Boolean) || (result.attachment ? [result.attachment] : []);
      if (attachments.length > 0) {
        setErrorNotice(null);
        appendPendingAttachments(attachments);
        if (result.error) setErrorNotice(result.error);
      }
    } catch (error) {
      setErrorNotice(String(error || '上传文件失败'));
    }
  }, [appendPendingAttachments, currentSessionId, isProcessing]);

  const pasteFilesAsAttachment = useCallback(async (files: File[]) => {
    if (isProcessing) return;
    try {
      setErrorNotice(null);
      if (files.length > 0) {
        const results = await Promise.all(files.map(async (file, index) => {
          ...
          return window.ipcRenderer.chat.createInlineAttachment({
            dataUrl,
            fileName: file.name || `clipboard-${Date.now()}-${index + 1}.png`,
            sessionId: currentSessionId || undefined,
          }) as Promise<{ success?: boolean; error?: string; attachment?: UploadedFileAttachment }>;
        }));
        const attachments = results.flatMap((result) => (
          result?.success && result.attachment ? [result.attachment] : []
        ));
        if (attachments.length === 0) {
          setErrorNotice(results.find((result) => result?.error)?.error || '粘贴图片失败');
          return;
        }
        appendPendingAttachments(attachments);
        const failures = results
          .filter((result) => !result?.success || !result.attachment)
          .map((result) => result?.error || '图片暂存失败');
        if (failures.length > 0) setErrorNotice(failures.join('；'));
        return;
      }

      const fileName = `clipboard-${Date.now()}.png`;
      {
        const clipboardImage = await window.ipcRenderer.invoke('clipboard:read-image') as {
          success?: boolean;
          dataUrl?: string;
          fileName?: string;
          error?: string;
        };
        if (!clipboardImage?.success || !clipboardImage.dataUrl) {
          setErrorNotice(clipboardImage?.error || '系统剪贴板中没有可粘贴的图片');
          return;
        }
        const result = await window.ipcRenderer.chat.createInlineAttachment({
          dataUrl: clipboardImage.dataUrl,
          fileName: clipboardImage.fileName || fileName,
          sessionId: currentSessionId || undefined,
        }) as { success?: boolean; error?: string; attachment?: UploadedFileAttachment };
        if (!result?.success || !result.attachment) {
          setErrorNotice(result?.error || '粘贴图片失败');
          return;
        }
        appendPendingAttachments([result.attachment]);
      }
    } catch (error) {
      setErrorNotice(error instanceof Error ? error.message : String(error || '粘贴图片失败'));
    }
  }, [appendPendingAttachments, currentSessionId, isProcessing]);
  ...
  const sendMessage = useCallback(async (
    ...
    const attachmentLabel = attachments.length === 1
      ? `请分析这个附件：${attachments[0].name}`
      : attachments.length > 1
        ? `请分析这 ${attachments.length} 个附件：${attachments.map((attachment) => attachment.name).join('、')}`
        : '';
    const displayText = normalizedContent || attachmentLabel;
    if (!displayText) return false;
    let targetSessionId = currentSessionId || undefined;
    let forkBaseMessages: Message[] | null = null;
    const activeForkDraft = options?.forkDraft || null;

    if (activeForkDraft) {
      try {
        const sourceTitle = sessions.find((session) => session.id === activeForkDraft.sourceSessionId)?.title || 'New Chat';
        const forkResult = await window.ipcRenderer.chat.forkFromMessage({
          sessionId: activeForkDraft.sourceSessionId,
          messageId: activeForkDraft.sourceMessageId,
          title: `${sourceTitle} · 分支`,
        });
        if (!forkResult?.success || !forkResult.session?.id) {
          throw new Error(forkResult?.error || '创建分支会话失败');
        }
        const forkSession = {
          id: forkResult.session.id,
          title: forkResult.session.title || `${sourceTitle} · 分支`,
          updatedAt: String(forkResult.session.updatedAt || Date.now()),
        };
        targetSessionId = forkSession.id;
        currentSessionIdRef.current = forkSession.id;
        setSessions((prev) => [forkSession, ...prev.filter((session) => session.id !== forkSession.id)]);
        setCurrentSessionId(forkSession.id);
        const sourceMessageIndex = messages.findIndex((message) => message.id === activeForkDraft.sourceMessageId);
        forkBaseMessages = sourceMessageIndex >= 0 ? messages.slice(0, sourceMessageIndex) : [];
      } catch (error) {
        setErrorNotice(error instanceof Error ? error.message : String(error));
        return false;
      }
    ...
    const userMsg: Message = {
      ...
      attachment: attachments[0] as Message['attachment'] | undefined,
      attachments,
      tools: [],
      timeline: []
    };
    ...
    const aiPlaceholder: Message = {
      ...
      content: '',
      tools: [],
      timeline: buildPendingAssistantTimeline('正在思考'),
      isStreaming: true,
      processingStartedAt,
    };
    ...
    setMessages(prev => (
      forkBaseMessages
        ? [...forkBaseMessages, userMsg, aiPlaceholder]
        : [...prev, userMsg, aiPlaceholder]
    ));
    if (!activeForkDraft) {
      setInput('');
    }
    setComposerPresetPrompt(null);
    if (!activeForkDraft) {
    ...
    const resolvedAttachments = applyAttachmentDeliveryModes(
      attachments,
      resolvedModelConfig?.modelName || getChatModelConfig()?.modelName,
    );
    ...
    dispatchChatSend({
      ...
      displayContent: displayText,
      attachments: stripTransientAttachmentPreviews(resolvedAttachments),
  ...
  const handleForkMessage = useCallback(async (message: Message, nextContent: string): Promise<boolean> => {
    const sourceSessionId = currentSessionIdRef.current;
    if (!sourceSessionId || message.role !== 'user') return false;
    const promptText = String(nextContent || '').trim();
    if (!promptText) return false;
    const forkAttachments = message.attachments?.length
      ? message.attachments
      : message.attachment?.type === 'uploaded-file'
        ? [message.attachment as UploadedFileAttachment]
        : [];
    return sendMessage(promptText, forkAttachments, {
      forkDraft: {
        sourceSessionId,
  ...
  const applyShortcut = useCallback((shortcut: ChatShortcut) => {
    ...
    if (action === 'inject') {
      setErrorNotice(null);
      if (shortcut.presetPrompt) {
        setComposerPresetPrompt({
          label: shortcut.label || '预设 Prompt',
          text: shortcut.text,
        });
        requestAnimationFrame(() => {
          composerRef.current?.focus();
  ...
  const buildComposerSubmitContent = useCallback(() => {
    const presetText = String(composerPresetPrompt?.text || '').trim();
    const userText = String(input || '').trim();
    return [presetText, userText].filter(Boolean).join('\n\n');
  }, [composerPresetPrompt, input]);
  ...
  const renderComposer = (
    ...
    <React.Fragment key={`composer-${source}-${variant}`}>
      <CliEscalationDialog
        key={`cli-escalation-${source}`}
        request={cliEscalationRequest}
        onApprove={handleApproveCliEscalation}
        onDeny={handleDenyCliEscalation}
      />
      <ToolConfirmDialog key={`tool-confirm-${source}`} request={confirmRequest} onConfirm={handleConfirmTool} onCancel={handleCancelTool} />
      <ChatComposer
        key={`chat-composer-${source}-${variant}`}
        ref={composerRef}
        theme={composerTheme}
        variant={variant}
        ...
        presetPrompt={composerPresetPrompt}
        onPresetPromptChange={setComposerPresetPrompt}
        onSubmit={() => sendMessage(buildComposerSubmitContent(), pendingAttachments)}
        placeholder={placeholder}
        attachments={pendingAttachments}
        onPickAttachment={allowFileUpload ? pickAttachment : undefined}
        onRemoveAttachment={removePendingAttachment}
        onPasteFiles={allowFileUpload ? pasteFilesAsAttachment : undefined}
  ...
  const welcomeShortcutsBlock = showWelcomeShortcuts && welcomeShortcuts.length > 0 ? (
    <div className="flex flex-wrap justify-center gap-2 text-xs">
      {welcomeShortcuts.map((shortcut, index) => (
        <button
  ...
  const handleWelcomeAction = useCallback(async (action: ChatWelcomeAction) => {
    if (action.onClick) {
      action.onClick();
      return;
    }
    ...
    if (action.text) {
      if (action.action === 'inject') {
        setErrorNotice(null);
        if (action.presetPrompt) {
          setComposerPresetPrompt({
            label: action.label || '预设 Prompt',
            text: action.text,
          });
          requestAnimationFrame(() => {
            composerRef.current?.focus();
  ...
  const headerWelcomeActionsBlock = welcomeActions && welcomeActions.length > 0 ? (
    <div className="flex items-center gap-1.5">
      {welcomeActions.map((action, index) => (
        <button
          key={`header-${action.label || 'action'}-${index}`}
          type="button"
          onClick={() => void handleWelcomeAction(action)}
          className={clsx(
            'inline-flex h-9 w-9 items-center justify-center rounded-full border shadow-sm backdrop-blur transition-colors active:scale-95',
            darkEmbedded
              ? 'border-white/10 bg-white/5 text-white/62 hover:border-white/20 hover:bg-white/10 hover:text-white'
              : 'border-border bg-surface-primary/80 text-text-tertiary hover:bg-surface-secondary hover:text-accent-primary',
          )}
          title={action.label}
          aria-label={action.label}
        >
  ...
  return (
    <div className={clsx('flex h-full min-w-0', wideContent && 'chat-layout-wide', narrowContent && 'chat-layout-narrow')}>
      ...
      <div className="flex-1 min-w-0 flex flex-col h-full relative overflow-hidden">
        ...
        {/* Header Actions */}
        {(headerWelcomeActionsBlock || (showClearButton && currentSessionId && messages.length > 0)) && (
          <div className="absolute top-4 right-4 z-10 flex items-center gap-2">
            {headerWelcomeActionsBlock}
                ...
                {dockedEmptyState ? (
                  ...
                  <>
                    {messages.map((msg, index) => (
                      <ErrorBoundary key={`${msg.id || msg.role || 'message'}-${index}`} name={`MessageItem-${msg.id || index}`}>
                        <MessageItem
                          msg={msg}
                          copiedMessageId={copiedMessageId}
                          onCopyMessage={handleCopyMessage}
                          onForkMessage={!fixedSessionId ? handleForkMessage : undefined}
                          onMcpAppMessage={(content) => sendMessage(content)}
                ...
                {showComposerShortcuts && shortcuts.length > 0 && (
                  <div className="flex gap-2 overflow-x-auto py-1 no-scrollbar">
                    {shortcuts.map((shortcut, index) => (
                      <button key={`${shortcut.label || 'shortcut'}-${index}`} onClick={() => applyShortcut(shortcut)} disabled={isProcessing} className={shortcutChipClass}>
                        {shortcut.label}
                      </button>
                    ))}
                  </div>
