...
type MediaSourceFilter = 'all' | 'generated' | 'imported' | 'external';
...
const CREATE_KIND_OPTIONS: Array<{ id: CreateKind; label: string; icon: typeof FileText; accentClass: string; available: boolean; unavailableHint?: string }> = [
    { id: 'longform', label: '长文', icon: FileText, accentClass: 'from-violet-500/20 via-accent-muted/70 to-surface-primary text-violet-600 dark:text-violet-300', available: false, unavailableHint: '暂未开放' },
    { id: 'richpost', label: '图文', icon: FileImage, accentClass: 'from-orange-500/20 via-status-warning/10 to-surface-primary text-orange-600 dark:text-orange-300', available: true },
    { id: 'video', label: '视频', icon: Clapperboard, accentClass: 'from-blue-500/20 via-accent-muted/70 to-surface-primary text-blue-600 dark:text-blue-300', available: false, unavailableHint: '正在开发中' },
    { id: 'audio', label: '音频', icon: AudioLines, accentClass: 'from-emerald-500/20 via-status-success/10 to-surface-primary text-emerald-600 dark:text-emerald-300', available: false, unavailableHint: '正在开发中' },
];
...
const MEDIA_SOURCE_OPTIONS: Array<{ id: MediaSourceFilter; label: string }> = [
    { id: 'all', label: '全部来源' },
    { id: 'generated', label: 'AI 生成' },
    { id: 'imported', label: '自有上传' },
    { id: 'external', label: '其他博主' },
];
...
function mediaFolderLabel(folder: FileNode): string {
    if (folder.path === 'generated') return 'AI 生成';
    if (folder.path === 'imported') return '自有上传';
    if (folder.path === 'external') return '其他博主';
    return folder.name;
}

function mediaSourceLabel(source: MediaAssetSource): string {
    if (source === 'generated') return 'AI';
    if (source === 'external') return '其他博主';
    if (source === 'imported') return '自有上传';
    return '计划素材';
}
...
function inferAssetKind(asset: MediaAsset): 'image' | 'video' | 'audio' | 'html' | 'pdf' | 'unknown' {
    ...
    if (mime === 'text/html' || /\.(html|htm)$/i.test(ref)) return 'html';
    if (mime === 'application/pdf' || /\.pdf$/i.test(ref)) return 'pdf';
    return 'unknown';
}
...
function sortMediaJobsByRecency(jobs: MediaJobProjection[]): MediaJobProjection[] {
    return [...jobs].sort((left, right) => {
        ...
        return (parseTimestampMs(right.createdAt) ?? 0) - (parseTimestampMs(left.createdAt) ?? 0);
    });
}
...
export function Manuscripts({ pendingFile, onFileConsumed, onNavigateToRedClaw, onNavigateToGenerationStudio, isActive = false, onImmersiveModeChange }: ManuscriptsProps) {
    ...
    const [mediaSourceFilter, setMediaSourceFilter] = useState<MediaSourceFilter>('all');
    const [mediaImportMenuOpen, setMediaImportMenuOpen] = useState(false);
    ...
    const pendingMediaFocusRef = useRef<{ assetId?: string; source?: string; at?: number } | null>(null);
    ...
    const handleImportMediaFiles = useCallback(async (source: 'imported' | 'external') => {
        setMediaImportMenuOpen(false);
        setWorkingId('media-import');
        try {
            const files = await window.ipcRenderer.command<string[]>('pick_files', {
                title: source === 'external' ? '选择其他博主素材' : '选择自有素材',
            });
            if (!Array.isArray(files) || files.length === 0) {
                return;
            }
            const result = await window.ipcRenderer.invoke('media:import-files', { files, source }) as {
                success?: boolean;
                canceled?: boolean;
                error?: string;
                added?: number;
    ...
    const loadSettings = useCallback(async () => {
        try {
            ...
            setSettings(next);
            setModel(next.image_model || 'gpt-image-2');
    ...
    useEffect(() => {
        if (!isActive) return;
        const queueFocus = (payload: { assetId?: string; source?: string; at?: number } | null | undefined) => {
            if (!payload?.assetId && !payload?.source) return;
            pendingMediaFocusRef.current = payload;
            setMode('gallery');
            setFilter('media');
            setActiveFolder('');
        };
        const handleFocus = (event: Event) => {
            queueFocus((event as CustomEvent<{ assetId?: string; source?: string; at?: number }>).detail);
        };
        window.addEventListener(MEDIA_LIBRARY_FOCUS_EVENT, handleFocus as EventListener);
        try {
            const stored = window.localStorage.getItem(MEDIA_LIBRARY_FOCUS_STORAGE_KEY);
            if (stored) queueFocus(JSON.parse(stored));
        } catch {
            // A live focus event can still complete navigation when local storage is unavailable.
        }
        return () => window.removeEventListener(MEDIA_LIBRARY_FOCUS_EVENT, handleFocus as EventListener);
    }, [isActive]);

    useEffect(() => {
        const pending = pendingMediaFocusRef.current;
        if (!isActive || !pending || assets.length === 0) return;
        const normalizedSource = String(pending.source || '').replace(/\\/g, '/').trim();
        const asset = assets.find((item) => item.id === pending.assetId)
            || assets.find((item) => {
                const candidates = [item.absolutePath, item.previewUrl, item.relativePath]
                    .map((value) => String(value || '').replace(/\\/g, '/').trim())
                    .filter(Boolean);
                return Boolean(normalizedSource) && candidates.some((candidate) => (
        ...
        setMediaFolder(getRelativeFolderPath(asset.relativePath || ''));
        setMediaSourceFilter('all');
        setPreviewAsset(asset);
        try {
            window.localStorage.removeItem(MEDIA_LIBRARY_FOCUS_STORAGE_KEY);
        } catch {
    ...
    const visibleAssets = useMemo(() => {
        ...
        return assets.filter((asset) => {
            ...
            if (filter === 'media' && !['image', 'video', 'audio', 'html', 'pdf', 'unknown'].includes(assetKind)) return false;
            ...
            if (mediaSourceFilter !== 'all' && asset.source !== mediaSourceFilter) return false;
    ...
    }, [activeFolder, assets, filter, isMediaScope, mediaFolder, mediaSourceFilter, normalizedQuery]);
    ...
    if (mode === 'editor' && editorFile) {
        ...
        const referencedTimelineAssets = timelineClips
        ...
        const packagePreviewAssets: MediaAsset[] = Array.from(new Map<string, MediaAsset>(
            [
                ...referencedTimelineAssets,
            ...
            <div className={clsx('h-full min-h-0 flex flex-col', isImmersiveWorkbench ? 'editor-ui-shell bg-background text-text-primary' : 'bg-surface-primary')}>
                <div className={clsx(
                    ...
                    isImmersiveWorkbench
                        ...
                        : 'border-b border-border bg-surface-primary/80 backdrop-blur-[32px]'
                )}>
        ...
        <div className="h-full min-h-0 overflow-auto bg-surface-primary text-text-primary">
                ...
                <div className="border-b border-black/[0.03] py-5 px-1">
                            ...
                            <div className="relative flex flex-wrap items-center gap-1.5">
                                <div className="flex items-center gap-0.5 bg-black/[0.03] p-1 rounded-xl border border-black/[0.02]">
                                    <button
                                        ...
                                        className={clsx(
                                            ...
                                            filter === 'folders' ? "bg-surface-primary text-text-primary shadow-sm ring-1 ring-border/50" : "text-text-tertiary hover:text-text-primary hover:bg-surface-secondary"
                                        )}
                                    >
                                ...
                                <div className="ml-2 flex items-center gap-1 rounded-xl border border-border bg-surface-elevated p-1 shadow-lg shadow-black/5">
                                    ...
                                    <div className="relative">
                                        <button
                                            type="button"
                                            onClick={() => setMediaImportMenuOpen((open) => !open)}
                                        ...
                                        >
                                            <Upload className="h-3.5 w-3.5" />
                                            {workingId === 'media-import' ? '导入中' : '上传素材'}
                                            <ChevronDown className="h-3 w-3" />
                                        </button>
                                        {mediaImportMenuOpen ? (
                                            <div className="absolute right-0 top-[calc(100%+8px)] z-50 w-44 rounded-xl border border-border bg-surface-elevated p-1.5 shadow-xl">
                                                <button
                                                    type="button"
                                                    onClick={() => void handleImportMediaFiles('imported')}
                                                    className="w-full rounded-lg px-3 py-2 text-left text-[12px] font-medium text-text-secondary hover:bg-surface-secondary hover:text-text-primary"
                                                >
                                                    上传自有素材
                                                </button>
                                                <button
                                                    type="button"
                                                    onClick={() => void handleImportMediaFiles('external')}
                                                    className="w-full rounded-lg px-3 py-2 text-left text-[12px] font-medium text-text-secondary hover:bg-surface-secondary hover:text-text-primary"
                                                >
                                                    上传其他博主素材
                                                </button>
                                ...
                                >
                                    <div className="rounded-[24px] border border-border bg-surface-elevated/95 p-4 shadow-[var(--ui-shadow-2)] backdrop-blur-[32px]">
                                        <div className="flex items-center gap-3 rounded-xl border border-border bg-surface-secondary/50 px-4 py-2.5 focus-within:bg-surface-primary focus-within:ring-2 focus-within:ring-accent-primary/10 transition-all">
                            ...
                            {visibleFolders.map((folder) => (
                                ...
                                >
                                    ...
                                    <div className="line-clamp-1 text-[11px] font-bold text-text-secondary group-hover:text-text-primary">
                                        {isMediaScope ? mediaFolderLabel(folder) : folder.name}
                                    </div>
                                </button>
                            ))}
                        </div>
                    ...
                    <div className="border-b border-black/[0.03] py-4 px-1">
                        <div className="flex flex-wrap items-center gap-2">
                                ...
                                {FILTER_OPTIONS.map((item) => (
                                    <button
                                        ...
                                        className={clsx(
                                            ...
                                            filter === item.id ? 'bg-surface-primary text-text-primary shadow-sm ring-1 ring-border/50' : 'text-text-tertiary hover:text-text-secondary hover:bg-surface-secondary'
                                        )}
                                    >
                                        {item.label}
                                    </button>
                            ...
                            {isMediaScope ? (
                                <div className="flex items-center gap-1 bg-black/[0.03] p-1 rounded-xl border border-black/[0.02]">
                                    {MEDIA_SOURCE_OPTIONS.map((item) => (
                                        <button
                                            key={item.id}
                                            type="button"
                                            onClick={() => setMediaSourceFilter(item.id)}
                                            className={clsx(
                                                'px-3 py-1.5 rounded-lg text-[11px] font-bold transition-all',
                                                mediaSourceFilter === item.id
                                                    ? 'bg-surface-primary text-text-primary shadow-sm ring-1 ring-border/50'
                                                    : 'text-text-tertiary hover:text-text-secondary hover:bg-black/[0.02]'
                                            )}
                                        >
                                            {item.label}
                                        </button>
                            ...
                            <div className="ml-auto flex items-center gap-0.5 bg-black/[0.03] p-1 rounded-xl border border-black/[0.02]">
                                <button
                                    ...
                                    className={clsx(
                                        ...
                                        layout === 'gallery' ? 'bg-surface-primary text-text-primary shadow-sm ring-1 ring-border/50' : 'text-text-tertiary hover:text-text-primary'
                                    )}
                                >
                                ...
                                <button
                                    ...
                                    className={clsx(
                                        'p-1.5 rounded-lg transition-all',
                                        layout === 'list' ? 'bg-surface-primary text-text-primary shadow-sm ring-1 ring-border/50' : 'text-text-tertiary hover:text-text-primary'
                                    )}
                                >
                                    <FolderOpen className="h-3.5 w-3.5" />
                                </button>
                                ...
                                <div className={clsx(layout === 'gallery' ? 'grid grid-cols-[repeat(auto-fill,minmax(180px,1fr))] gap-3.5' : 'space-y-2')}>
                                    {visibleFolders.map((folder) => (
                                        <button
                                            ...
                                            onContextMenu={isMediaScope ? undefined : (event) => openFolderContextMenu(event, folder)}
                                            className={clsx(
                                                'rounded-2xl border border-border bg-surface-primary/70 text-left hover:bg-surface-primary',
                                        ...
                                        >
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div className="min-w-0">
                                                    <div className="truncate text-sm font-medium text-text-primary">{isMediaScope ? mediaFolderLabel(folder) : folder.name}</div>
                                    ...
                                    <div className={clsx(layout === 'gallery' ? 'grid grid-cols-[repeat(auto-fill,minmax(176px,1fr))] gap-x-4 gap-y-6' : 'space-y-2')}>
                                        {contentCards.map((card) => {
                                            if (card.kind === 'draft') {
                                                ...
                                                return (
                                                    <div key={card.id} className={clsx(layout === 'gallery' ? 'group' : 'rounded-xl border border-border bg-surface-primary/70 px-4 py-2.5 transition-all hover:bg-surface-primary hover:shadow-sm')}>
                                                        {layout === 'gallery' ? (
                                                            <div
                                                                role="button"
                                                                tabIndex={0}
                                                            ...
                                                            >
                                                                <div className={clsx(
                                                                    'relative aspect-[5/6] overflow-hidden rounded-[14px] border border-border bg-surface-primary transition-all duration-300 group-hover:-translate-y-1 group-hover:shadow-[var(--ui-shadow-1)] focus-visible:ring-2 focus-visible:ring-accent-primary/30',
                                                                    ...
                                                                    {showRichpostPreview ? (
                                                                        <img
                                                                            ...
                                                                            loading="lazy"
                                                                            draggable={false}
                                                                            className="theme-content-light h-full w-full select-none object-cover pointer-events-none bg-white"
                                                                        />
                                                                    ) : (
                                            ...
                                            const thumbnailSrc = resolveAssetUrl(asset.thumbnailUrl || '');
                                                        ...
                                                        {layout === 'gallery' ? (
                                                            <>
                                                                <div className="relative aspect-[5/6] overflow-hidden rounded-[14px] border border-black/[0.04] bg-black/[0.02] transition-all duration-300 group-hover:-translate-y-1 group-hover:shadow-[0_12px_24px_-8px_rgba(0,0,0,0.12)]">
                                                                    <div className="absolute left-2.5 top-2.5 z-10 rounded-lg bg-black/60 px-2 py-1 text-[9px] font-bold text-white backdrop-blur-md border border-white/10">
                                                                    ...
                                                                    {assetKind === 'image' ? (
                                                                        ...
                                                                        <>
                                                                            {thumbnailSrc ? (
                                                                                <img src={thumbnailSrc} alt={`${asset.title || asset.id} 视频首帧`} className="h-full w-full object-cover bg-black" />
                                                                            ) : (
                                                                                <video
                                                                                    src={previewSrc}
                                                                        ...
                                                                        <div className="flex h-full w-full items-center justify-center bg-[linear-gradient(135deg,#10253f,#315e8f)] text-white/80">
                                                                            {assetKind === 'audio' ? <AudioLines className="h-9 w-9" /> : <FileText className="h-9 w-9" />}
                                                                        </div>
                                                                    )}
                                                                ...
                                                                <div className="h-10 w-12 overflow-hidden rounded-lg bg-black/[0.03] border border-black/[0.04]">
                                                                    {assetKind === 'image' ? (
                                                                        ...
                                                                        thumbnailSrc ? (
                                                                            ...
                                                                            <video src={previewSrc} className="h-full w-full object-cover bg-black" muted playsInline preload="auto" />
                                                                        )
                                                                    ) : assetKind === 'html' || assetKind === 'pdf' ? (
                                                                        <div className="flex h-full w-full items-center justify-center bg-surface-primary text-text-tertiary">
                                                                            <FileText className="h-4 w-4" />
                                                                        </div>
                                                                    ) : (
                                                                        <div className="flex h-full w-full items-center justify-center bg-black/[0.05] text-text-tertiary">
                                                                            {assetKind === 'audio' ? <AudioLines className="h-4 w-4" /> : <FileText className="h-4 w-4" />}
                                                                        </div>
                                                                    )}
                                                                </div>
                                                                <div className="min-w-0 flex-1">
                    ...
                    <div className="w-full max-w-5xl max-h-[88vh] overflow-auto rounded-2xl border border-border bg-surface-primary shadow-2xl">
                        ...
                        <div className="p-5 space-y-4">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
                                ...
                                <input value={model} onChange={(event) => setModel(event.target.value)} placeholder="模型（如 gpt-image-2）" className="px-3 py-2 text-sm rounded-md border border-border bg-surface-secondary/20 focus:outline-none focus:ring-1 focus:ring-accent-primary" />
                        ...
                        <div className="grid max-h-[72vh] grid-cols-[repeat(auto-fill,minmax(160px,1fr))] gap-4 overflow-auto p-5">
                            {bindableAssets.map((asset) => (
                                <button
                                    ...
                                    className="overflow-hidden rounded-2xl border border-border bg-surface-primary text-left hover:border-accent-primary/40"
                                >
            ...
            {isVideoModalOpen && (
                <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/45 p-4">
                    <div className="w-full max-w-5xl max-h-[88vh] overflow-auto rounded-2xl border border-border bg-surface-primary shadow-2xl">
                        ...
                        <div className="p-5 space-y-4">
                            <div className="text-xs text-text-secondary">
                                当前生视频配置：source=<span className="font-mono">商媒运营助手官方</span> · model=<span className="font-mono">{effectiveVideoModel}</span> · endpoint=<span className="font-mono">{resolvedVideoEndpoint || '(未设置)'}</span>
                            </div>
                            {!hasVideoConfig && (
                                <div className="text-xs text-status-error">
                                    未检测到可用的商媒运营助手官方视频配置。请先登录或配置商媒运营助手官方 AI 源。
                                </div>
                            )}
            ...
            {createOpen && (
                <div className="fixed inset-0 z-[1000] bg-black/40 backdrop-blur-[4px] flex items-center justify-center p-4 animate-in fade-in duration-300" onMouseDown={() => !isCreating && setCreateOpen(false)}>
                    <div className="w-full max-w-[680px] rounded-[24px] border border-border bg-surface-elevated shadow-[var(--ui-shadow-2)] flex flex-col overflow-hidden" onMouseDown={(event) => event.stopPropagation()}>
                        <div className="flex items-center justify-between px-6 py-5 border-b border-divider bg-surface-elevated">
                        ...
                        <div className="bg-surface-secondary/50 px-5 py-5">
                                ...
                                {CREATE_KIND_OPTIONS.map((option) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={clsx(
                                                'group relative flex min-h-[124px] flex-col justify-between rounded-[18px] border border-border bg-surface-primary p-4 text-left transition-all duration-200',
                                                !isUnavailable && 'hover:border-accent-primary/40 hover:shadow-md',
                                        ...
                                        >
                                            <div className={clsx('relative overflow-hidden rounded-[16px] border border-border/70 bg-gradient-to-br p-3 transition-transform duration-200', option.accentClass, !isUnavailable && 'group-hover:scale-[1.02]')}>
                                                <div className="absolute inset-x-3 bottom-2 h-5 rounded-full bg-surface-primary/60 blur-md" />
                                                <div className="relative flex h-11 w-11 items-center justify-center rounded-[14px] bg-surface-primary/75 shadow-sm">
                                            ...
                                            {isUnavailable && option.unavailableHint ? (
                                                <div className="pointer-events-none absolute left-1/2 top-3 z-10 -translate-x-1/2 -translate-y-1 opacity-0 transition-all duration-150 group-hover:translate-y-0 group-hover:opacity-100">
                                                    <div className="rounded-full bg-text-primary px-3 py-1 text-[11px] font-bold text-background shadow-[0_10px_24px_-12px_rgba(0,0,0,0.45)]">
            ...
            {folderCreateOpen && (
                <div className="fixed inset-0 z-[1000] bg-black/10 backdrop-blur-[6px] flex items-center justify-center p-4 animate-in fade-in duration-300" onMouseDown={() => !isCreating && setFolderCreateOpen(false)}>
                    <div className="w-full max-w-md rounded-[24px] border border-border bg-surface-elevated/95 shadow-[var(--ui-shadow-2)] backdrop-blur-[32px] overflow-hidden flex flex-col" onMouseDown={(event) => event.stopPropagation()}>
                        ...
                        <div className="px-7 py-2 space-y-4">
                            <input
                                ...
                                placeholder="输入文件夹名称..."
                                className="w-full rounded-xl border border-border bg-surface-secondary/50 px-4 py-3 text-sm font-bold placeholder:text-text-tertiary/50 focus:outline-none focus:bg-surface-primary focus:ring-2 focus:ring-accent-primary/10 transition-all"
                            />
                        </div>
                        ...
                        <div className="h-[72vh] bg-black/90 flex items-center justify-center p-6">
                            {inferAssetKind(previewAsset) === 'image' ? (
                                ...
                                <video
                                    ...
                                    poster={resolveAssetUrl(previewAsset.thumbnailUrl || '') || undefined}
                                    controls
                                    className="w-full max-h-full rounded-2xl bg-black"
                                />
