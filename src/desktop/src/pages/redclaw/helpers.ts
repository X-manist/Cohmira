...
export function buildRedClawSessionMetadata() {
    return {
        agentRuntime: 'goose',
        runtimeBridge: 'goose',
        runtimeMode: 'redclaw',
...
export function compareContextSessionItems(
    ...
    const leftUpdatedAt = String(left.chatSession?.updatedAt || '');
    const rightUpdatedAt = String(right.chatSession?.updatedAt || '');
