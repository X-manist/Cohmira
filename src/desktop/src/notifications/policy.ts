...
function summarizeRuntimeMode(runtimeMode: string): string {
  if (runtimeMode === 'redclaw') return '商媒运营助手';
...
function shouldMuteSuccessForForeground(context: NotificationContextSnapshot, source: string): boolean {
  ...
  if (source === 'generation' && context.currentView === 'redclaw') return true;
  ...
  const notification: NotificationEnvelope = {
    ...
    title: isCompleted
      ...
      : needsConfirmation
        ? `商媒运营助手任务需要确认`
        : `商媒运营助手任务失败`,
    body: event.summary || event.taskName || '商媒运营助手后台任务状态发生变化。',
    sound: 'none',
