import { convertFileSrcWithTauri, getElectronIpcTransport } from './electronTransport';
...
const nativeTauriCommands = new Set([
  'pick_directory',
  'pick_files',
]);
...
export async function invoke<T = unknown>(
  ...
  if (nativeTauriCommands.has(command)) {
...
export function convertFileSrc(pathValue: string): string {
  const tauriUrl = convertFileSrcWithTauri(pathValue);
  if (tauriUrl) {
    return tauriUrl;
  }
