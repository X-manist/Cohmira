...
这个目录提供商媒运营助手浏览器插件的工程化构建源码，用来把外部网页内容采集到商媒运营助手桌面端知识库和素材库。
...
3. 打开“开发者模式”。
4. 点击“加载已解压的扩展程序”。
5. 选择当前仓库里的 [Plugin/dist/extension](/Volumes/macsoftware/codes/agentscompany/yunyingagent/src/Plugin/dist/extension) 目录。

源码在 [src](/Volumes/macsoftware/codes/agentscompany/yunyingagent/src/Plugin/src) 目录。`dist/extension` 是构建产物，不要手改。
...
- App 内置 MCP：桌面端启动时会自动注册商媒运营助手浏览器控制 MCP server，stdio command 指向 App 自身的隐藏兼容 `--redbox-browser-control-mcp` 模式，不要求用户手动导入 MCP 配置。
...
- Agent-side JS client：`scripts/browser-client.mjs` 提供 Codex 同款 `setupBrowserRuntime({ globals }) -> agent.browsers.get("extension")` 入口，用对象方法包装 `browser.user.openTabs()`、`browser.tabs.new()`、`tab.playwright.locator()`、`browser.tabs.finalize()` 等能力；底层仍走商媒运营助手 native-host socket / MCP 工具。
...
不传 `--extension-id` 时，安装器会尝试从 Chrome / Edge / Brave 等 profile 里发现已加载的商媒运营助手 unpacked extension。`--node` 默认使用当前 Node；面向真实 Google Chrome 时建议传绝对 Node 22 路径，避免 GUI Chrome 的 PATH 与终端不同。
...
{
  "command": "node",
  "args": ["/Volumes/macsoftware/codes/agentscompany/yunyingagent/src/Plugin/mcp-server.mjs"]
}
```

插件根目录也提供 [Plugin/.mcp.json](/Volumes/macsoftware/codes/agentscompany/yunyingagent/src/Plugin/.mcp.json)，用于开发态本地发现或外部 MCP 客户端导入 `browser-control` server；正式 App 运行时优先使用内置 MCP。
...
- “打开网页读取内容”不是浏览器控制验收；必须看到商媒运营助手 MCP / native host socket 经真实 Chrome 扩展返回 `tools/list`、`tabs.list`、`tab.info`、DOM 查询和至少一个交互动作。
- `pnpm smoke:browser-control` 使用临时 profile / Chromium 做回归，不代表用户真实 Chrome 可用。
- 真实 Chrome 验收必须使用已安装的商媒运营助手扩展、真实 Chrome native messaging manifest、真实 socket，以及真实标签页或受控测试标签页。
- 被 `tab.claim` / `tab.create` 纳入 active browser session 的页面必须显示 `商媒运营助手控制中` 页面内标签；释放、finalize 或 turn 结束后自动移除。
...
- `scripts/browser-client.mjs`：供 agent / 调试脚本按 Codex Browser Use 对象 API 使用商媒运营助手 browser-control；配套文档在 `Plugin/docs/browser-runtime.md`。
- `pnpm install:native-host`：安装 Chrome native messaging host manifest。
- `pnpm diagnose:browser-control`：检查 native host manifest、launcher、商媒运营助手 endpoint socket 和 extension forwarding 状态；需要只取报告时加 `-- --no-fail`。
...
- `pnpm mcp:server`：启动开发态商媒运营助手 browser-control stdio MCP server；正式 App 使用内置 Rust MCP 入口。
- `pnpm package`：先构建，再生成插件发布包。
...
- 点击浏览器扩展图标会打开商媒运营助手侧边栏，不再使用 popup。
- 可在扩展详情页点击“扩展程序选项”，或在侧边栏顶部点击设置按钮，打开插件设置页。
- 侧边栏展示当前页面识别、统一任务队列和批量采集入口；详情页采集、下载、导出等轻操作仍通过网页内 DOM 注入按钮触发。
- 在小红书详情页可使用笔记操作区注入按钮：商媒运营助手保存、下载压缩包、下载素材、采集评论。
- 在已注入页面右侧可使用商媒运营助手浮动采集面板；小红书笔记页、YouTube、抖音、公众号和普通网页会显示不同动作。
...
- 在任意网页中选中文字，右键点击“保存选中文字到商媒运营助手”
- 在任意网页使用右侧浮动采集面板保存当前页面链接
- 检测到新版本后，点击“打开更新源”会打开商媒运营助手下载源，下载插件压缩包后重新加载扩展即可完成更新

## 备注

- 插件负责采集、下载、导出、提交结构化数据，以及为桌面端 AI 暴露浏览器控制 MCP 工具；AI 编排和业务决策仍在商媒运营助手桌面端完成。
...
- 知识整理、漫步和创作任务仍在桌面端完成。
- 自动更新检查会在插件安装、浏览器启动和后台定时任务中执行；当前兼容更新源以插件构建配置为准。
