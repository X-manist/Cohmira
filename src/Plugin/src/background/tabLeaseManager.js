...
export async function groupFinalizedTabs(finalized) {
  ...
  if (handoff.length) {
    ...
    await chrome.tabGroups.update(groupId, { title: 'Cohmira Handoff', color: 'blue' }).catch(() => {});
  }
  if (deliverable.length) {
    const groupId = await chrome.tabs.group({ tabIds: deliverable });
    await chrome.tabGroups.update(groupId, { title: 'Cohmira Deliverable', color: 'green' }).catch(() => {});
  }
}
