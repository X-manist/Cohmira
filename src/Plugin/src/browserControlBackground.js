...
async function resolveApiBase(force = false) {
  ...
  cachedBaseUrl = null;
  throw new Error(lastError || 'Cohmira browser-control HTTP bridge is not configured');
}
...
async function ensureAppServerWithSidePanelGate(message = {}) {
  ...
  try {
    const sidePanel = await requireSidePanelOpen(message.windowId || message.params?.windowId, {
      closedError: isTargetCodexAlias ? 'Codex side panel is not open.' : 'Cohmira side panel is not open.',
    });
