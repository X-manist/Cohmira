...
function renderConnection(health) {
  if (!health?.success) {
    elements.serverStatus.textContent = '未链接，请打开商媒运营助手';
...
function renderWorkspaceAndAccounts(nextContext, healthPayload) {
  ...
  if (!healthPayload?.success) {
    ...
    elements.boundAccountName.textContent = '未连接商媒运营助手';
    return;
  }
...
function renderPageIdentity(view) {
  ...
  elements.platformName.textContent = view.name || '商媒运营助手';
...
function renderCaptureActions(nextContext) {
  ...
  if (!isHealthy) {
    renderCaptureStatus('未链接，请打开商媒运营助手', 'error');
    return;
  }
...
function getCaptureActionConfig(nextContext) {
  ...
  const platformMap = {
    bilibili: { subtitle: 'Bilibili', label: pageType === 'video' ? '保存视频' : '保存页面', action: 'saveBilibili', title: '保存当前 Bilibili 内容到商媒运营助手' },
    kuaishou: { subtitle: '快手', label: pageType === 'video' ? '保存视频' : '保存页面', action: 'saveKuaishou', title: '保存当前快手内容到商媒运营助手' },
    tiktok: { subtitle: 'TikTok', label: pageType === 'video' ? '保存视频' : '保存页面', action: 'saveTiktok', title: '保存当前 TikTok 内容到商媒运营助手' },
    reddit: { subtitle: 'Reddit', label: pageType === 'post' ? '保存帖子' : '保存页面', action: 'saveReddit', title: '保存当前 Reddit 内容到商媒运营助手' },
    x: { subtitle: 'X', label: pageType === 'post' ? '保存推文' : '保存页面', action: 'saveX', title: '保存当前 X 内容到商媒运营助手' },
    instagram: { subtitle: 'Instagram', label: pageType === 'post' || pageType === 'video' ? '保存内容' : '保存页面', action: 'saveInstagram', title: '保存当前 Instagram 内容到商媒运营助手' },
  };
...
function summarizeActionResponse(response, fallback) {
  if (response?.noteId) {
    return response.duplicate ? '知识库中已存在' : '已保存到商媒运营助手';
  }
...
function getPlatformMeta(platform) {
  const map = {
    ...
    redbox: { platform: 'redbox', name: '商媒运营助手', logo: 'C' },
