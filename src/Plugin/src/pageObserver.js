      ...
      <style>
        ...
        .meta {
	      ...
	      <div class="zone" data-visible="false" data-state="idle">
	        <div class="eyebrow">商媒运营助手</div>
	        <div class="title">保存图片到商媒运营助手</div>
...
function setDragZoneState(state, payload, message) {
    ...
    const title = normalizeText(payload?.title) || '保存图片到商媒运营助手';
    ...
    dragZoneTitleElement.textContent = '保存图片到商媒运营助手';
    dragZoneMetaElement.textContent = message || title;
}
...
async function runXhsDomAction(action, options = {}) {
    ...
    const actionMap = {
        save: { type: 'save-xhs', pending: '保存中...', done: '已保存到商媒运营助手' },
        ...
        savePageAuto: { type: 'save-page-auto', pending: '保存中...', done: '已保存到商媒运营助手' },
        savePageLink: { type: 'save-page-link', pending: '保存中...', done: '已保存到商媒运营助手' },
...
function createXhsDetailHost(injectionKey) {
    ...
    saveButton.title = '保存当前小红书笔记到商媒运营助手';
...
function injectXhsProfileActions() {
    ...
    if (ACCOUNT_BINDING_FEATURE_ENABLED) {
        container.append(makeXhsDomButton('保存博主', 'blogger', { primary: true, statusTarget: container, title: '保存当前小红书博主资料到商媒运营助手' }));
    }
    container.append(
...
function getRedboxOverlayConfig(pageInfo) {
    ...
    if (isXhsNoteDetailPath() || /^xhs-(note|image|video)$/i.test(String(pageInfo?.kind || ''))) {
        return {
            ...
            subtitle: '小红书笔记页',
            actions: [
                { label: '保存笔记', action: 'save', primary: true, title: '保存当前笔记到商媒运营助手' },
    ...
    if (isXhsHost()) {
        return {
            ...
            subtitle: '当前页面',
            actions: [
                { label: '保存网页', action: 'savePageLink', primary: true, title: '保存当前页面链接到商媒运营助手' },
            ],
        };
    }
    if (pageInfo?.kind === 'youtube') {
        return {
            ...
            subtitle: 'YouTube',
            actions: [
                { label: '保存视频', action: 'saveYoutube', primary: true, title: '保存当前 YouTube 视频到商媒运营助手' },
            ],
        };
    }
    if (pageInfo?.kind === 'douyin-video') {
        return {
            ...
            subtitle: '抖音',
            actions: [
                { label: '保存视频', action: 'saveDouyin', primary: true, title: '保存当前抖音视频到商媒运营助手' },
            ],
        };
    }
    const platformMap = {
        'bilibili': { variant: 'bilibili', subtitle: 'Bilibili', label: '保存内容', action: 'saveBilibili', title: '保存当前 Bilibili 内容到商媒运营助手' },
        'kuaishou': { variant: 'kuaishou', subtitle: '快手', label: '保存内容', action: 'saveKuaishou', title: '保存当前快手内容到商媒运营助手' },
        'tiktok': { variant: 'tiktok', subtitle: 'TikTok', label: '保存内容', action: 'saveTiktok', title: '保存当前 TikTok 内容到商媒运营助手' },
        'reddit': { variant: 'reddit', subtitle: 'Reddit', label: '保存内容', action: 'saveReddit', title: '保存当前 Reddit 内容到商媒运营助手' },
        'x': { variant: 'x', subtitle: 'X', label: '保存内容', action: 'saveX', title: '保存当前 X 内容到商媒运营助手' },
        'instagram': { variant: 'instagram', subtitle: 'Instagram', label: '保存内容', action: 'saveInstagram', title: '保存当前 Instagram 内容到商媒运营助手' },
    };
    ...
    if (pageInfo?.kind === 'wechat-article') {
        return {
            ...
            subtitle: '微信公众号',
            actions: [
                { label: '保存文章', action: 'savePageLink', primary: true, title: '保存当前公众号文章到商媒运营助手' },
            ],
        };
    }
    if (pageInfo?.kind === 'zhihu-answer') {
        return {
            ...
            subtitle: '知乎',
            actions: [
                { label: '保存回答', action: 'saveZhihuAnswer', primary: true, title: '保存当前知乎回答到商媒运营助手' },
            ],
        };
    }
    if (pageInfo?.kind === 'zhihu-article') {
        return {
            ...
            subtitle: '知乎专栏',
            actions: [
                { label: '保存文章', action: 'saveZhihuArticle', primary: true, title: '保存当前知乎专栏文章到商媒运营助手' },
            ],
        };
    }
    return {
        ...
        actions: [
            { label: '保存网页', action: pageInfo?.action === 'save-page-auto' ? 'savePageAuto' : 'savePageLink', primary: true, title: '保存当前网页到商媒运营助手' },
        ],
    };
}
...
function ensureXhsOverlay(pageInfo) {
    ...
    shadow.innerHTML = `
      ...
      <div class="dock" role="toolbar" aria-label="商媒运营助手页面采集">
      </div>
    `;
