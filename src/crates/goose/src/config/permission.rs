...
impl PermissionManager {
    ...
    /// Isolated in-memory permission manager for background agents.
    /// It never reads or writes the user's global permission file.
    pub fn new_ephemeral() -> Self {
        Self {
            config_path: PathBuf::new(),
            permission_map: RwLock::new(HashMap::new()),
            persist: false,
        }
    }

    /// Atomically replaces the background agent's complete user tool policy.
    pub fn replace_user_permissions(&self, permissions: &[(String, PermissionLevel)]) {
        let mut config = PermissionConfig::default();
        for (name, level) in permissions {
            match level {
                PermissionLevel::AlwaysAllow => config.always_allow.push(name.clone()),
                PermissionLevel::AskBefore => config.ask_before.push(name.clone()),
                PermissionLevel::NeverAllow => config.never_allow.push(name.clone()),
            }
        }
        self.permission_map
            .write()
            .unwrap()
            .insert(USER_PERMISSION.to_string(), config);
    }
...
mod tests {
    ...
    #[test]
    fn ephemeral_permissions_are_complete_and_never_touch_disk() {
        let manager = PermissionManager::new_ephemeral();
        manager.replace_user_permissions(&[
            ("allowed".into(), PermissionLevel::AlwaysAllow),
            ("denied".into(), PermissionLevel::NeverAllow),
        ]);
        assert_eq!(
        ...
        assert!(manager.get_config_path().as_os_str().is_empty());
    }
