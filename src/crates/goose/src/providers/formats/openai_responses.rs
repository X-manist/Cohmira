...
fn parse_responses_stream_event(data_line: &str) -> anyhow::Result<Option<ResponsesStreamEvent>> {
    let raw_event: Value = serde_json::from_str(data_line).map_err(|e| {
        anyhow!(
            "Failed to parse Responses stream event JSON: {} (payload {} bytes)",
            e,
            data_line.len()
        )
    })?;
    ...
    let event = serde_json::from_value(raw_event).map_err(|e| {
        anyhow!(
            "Failed to parse Responses stream event type {}: {} (payload {} bytes)",
            event_type,
            e,
            data_line.len()
        )
        ...
        match item {
            ...
            ResponseOutputItemInfo::FunctionCall {
                ...
                let request_id = call_id
                    .or(id)
                    .unwrap_or_else(|| format!("tool-call-{}", content.len() + 1));
                let parsed_args = if arguments.is_empty() {
                    json!({})
                } else {
                    serde_json::from_str(&arguments).unwrap_or_else(|_| json!({}))
...
mod tests {
    ...
    #[tokio::test]
    async fn test_responses_stream_completed_allows_message_without_id_or_status(
    ) -> anyhow::Result<()> {
        let lines = vec![
            r#"data: {"type":"response.created","sequence_number":1,"response":{"id":"resp_1","object":"response","created_at":1737368310,"status":"in_progress","model":"gpt-5.6-sol","output":[]}}"#.to_string(),
            r#"data: {"type":"response.output_text.delta","sequence_number":2,"item_id":"msg_1","output_index":0,"content_index":0,"delta":"OK"}"#.to_string(),
            r#"data: {"type":"response.completed","sequence_number":3,"response":{"id":"resp_1","object":"response","created_at":1737368310,"status":"completed","model":"gpt-5.6-sol","output":[{"type":"message","role":"assistant","content":[{"type":"output_text","text":"OK"}]}],"usage":{"input_tokens":10,"output_tokens":2,"total_tokens":12}}}"#.to_string(),
            "data: [DONE]".to_string(),
        ];
        ...
        assert_eq!(text_parts.concat(), "OK");
        assert_eq!(usage.expect("usage should be present").model, "gpt-5.6-sol");
        Ok(())
    }
    ...
    #[test]
    fn test_completed_event_accepts_idless_function_call() {
        let json = r#"{
                ...
                "output": [
                    {
                        "type": "message",
                        "role": "assistant",
                        "content": [{"type": "output_text", "text": "Calling the image tool."}]
                    },
                    {
                        "type": "function_call",
                        "call_id": "call_image_1",
                        "name": "yunying-ops__generate_image",
        ...
        let event = parse_responses_stream_event(json).unwrap().unwrap();
        let ResponsesStreamEvent::ResponseCompleted { response, .. } = event else {
            panic!("expected response.completed");
        };
        let content = process_streaming_output_items(response.output, false);
        assert_eq!(content.len(), 2);
        match &content[1] {
            MessageContent::ToolRequest(request) => {
                assert_eq!(request.id, "call_image_1");
            }
    ...
    #[test]
    fn test_stream_parse_error_does_not_echo_provider_payload() {
        let secret = "sensitive-tool-schema-must-not-reach-ui";
        let json = format!(
            r#"{{"type":"response.completed","sequence_number":1,"response":{{"id":"resp_1","object":"response","created_at":1,"status":"completed","model":"gpt-5.5","output":[{{"type":"function_call","call_id":"call_1","arguments":"{{}}","private":"{secret}"}}]}}}}"#
        );

        let error = parse_responses_stream_event(&json).unwrap_err().to_string();
        assert!(error.contains("response.completed"));
        assert!(error.contains("payload"));
        assert!(!error.contains(secret));
        assert!(!error.contains("function_call"));
    }
