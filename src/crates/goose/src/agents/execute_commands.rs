...
static COMMANDS: &[CommandDef] = &[
    CommandDef {
        name: "help",
        description: "List built-in, recipe, and skill slash commands",
    },
    CommandDef {
        name: "prompts",
    ...
    CommandDef {
        name: "loop",
        description:
            "Alias for /grind: keep pursuing a goal until max_turns, or clear with /loop off",
    },
];
...
impl Agent {
    pub async fn execute_command(
        ...
        match command {
            "help" => self.handle_help_command(session_id).await,
            ...
            "grind" | "loop" => self.handle_grind_command(params_str).await,
            _ => {
    ...
    async fn handle_help_command(&self, session_id: &str) -> Result<Option<Message>> {
        ...
        let commands =
            crate::slash_commands::slash_command::list_acp_commands(working_dir.as_deref());
        let mut output = String::from("**Available slash commands**\n\n");
        for command in commands {
            output.push_str(&format!("- `/{}` — {}", command.name, command.description));
            if let Some(input_hint) = command.input_hint.filter(|hint| !hint.trim().is_empty()) {
                output.push_str(&format!(" (`{input_hint}`)"));
            }
            output.push('\n');
        }
        output.push_str("\nType `/` in the composer to search and insert a command.");
...
mod tests {
    ...
    #[test]
    fn builtin_commands_include_help_and_loop_alias() {
        let names: Vec<_> = list_commands().iter().map(|command| command.name).collect();
