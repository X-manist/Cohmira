...
| 本地文档 | 纯 Rust 只读解析 PDF、DOCX/DOCM、PPTX/PPTM、XLSX/XLSM，无需 Python、Office、LibreOffice 或 Poppler |
...
- 文档作为聊天附件上传后，员工端会在本机提取正文、表格单元格、幻灯片及备注；知识库导入的文档可由同一个只读工具按需解析。Agent 仅获得已授权路径和有长度上限的提取文本。
- 解析器不执行宏、嵌入对象或外部链接；单文件上限为 100 MB，并对 OOXML 解压大小和压缩率设置限制。
- 扫描版 PDF 没有嵌入文本时需要另行 OCR；旧版 `.doc`、`.ppt` 需先另存为 `.docx`、`.pptx`，`.xls`、`.xlsb`、`.ods` 需先另存为 `.xlsx`。该能力用于语义读取，不等同于 Microsoft Office 的高保真排版预览。
...
Tauri 只会把工作区中的 `media`、`cover` 和 `manuscripts` 目录加入只读预览范围；`knowledge`、`redclaw/uploads` 等知识与文档目录不会暴露给 WebView。默认 `~/.redconvert` 下的空间可直接切换；修改 `workspace_dir` 或在自定义工作区切换空间后需重启桌面应用，避免旧目录授权在进程内累积。
