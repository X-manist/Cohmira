...
Example: Before calling `seedance_video`, read its `agent_skills` → `seedance-2-0` and `ai-video-gen`. The call itself always routes through JiubanAI's app-managed video service.
...
| **Video generation** | `seedance-2-0`, `ai-video-gen`, `ark-plan-video-director`; execution is restricted to JiubanAI `app_cli video generate` |
