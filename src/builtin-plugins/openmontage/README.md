...
> JiubanAI built-in plugin: install/import this whole directory as one plugin. Python is managed by the app-bundled uv runtime. Video generation is restricted to JiubanAI's built-in Volcengine service through `video_selector` / `seedance_video`; upstream provider setup examples below are historical reference and are not active in this distribution.
...
## JiubanAI AI Short-drama Workflow

This distribution adds a Python-native MCP runtime and a single interactive MCP App for episodic AI drama.

```text
User intent
  → host AI reads the ai-short-drama Skills
  → Python MCP validates and versions artifacts
  → MCP App collects character/storyboard/asset/voice choices
  → existing OpenMontage tools generate, edit, compose, and verify media
```

The UI is not generated ad hoc by the model. The model explains and recommends; the deterministic workspace at
`ui://openmontage/short-drama` renders project state and commits binding user decisions.

Primary tools:

- `drama_project_create` — create a project and optionally import source text.
- `drama_stage_context` — load current revision, inputs, outputs, and the required stage Skill.
- `drama_artifact_save` — validate and persist AI-produced artifacts.
- `drama_open_review` — open the interactive workspace.
- `drama_selection_commit` / `drama_stage_decide` — persist choices and approvals.
- Existing `list_tools`, `provider_menu`, and `run_tool` — execute the original OpenMontage media toolkit.

For local contract testing, use the same runtime shape as the desktop app:

```bash
UV_CACHE_DIR=/tmp/openmontage-uv-cache \
uv run --managed-python --python 3.11 --no-project \
  --with-requirements requirements.txt mcp/server.py
```

The desktop app supplies its bundled `uv`; end users do not install Python or pip. Project data is stored outside
the installed plugin under `OPENMONTAGE_PROJECTS_DIR`, so plugin upgrades do not overwrite user productions.

See `docs/AI_SHORT_DRAMA.md` for the operating contract.
