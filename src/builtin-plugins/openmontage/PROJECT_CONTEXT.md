...
- **Capability-first tool design:** Each major family exposes a selector tool; generated video is intentionally single-provider
  - Example: `tts_selector` + `elevenlabs_tts` / `google_tts` / `openai_tts` / `piper_tts`
  - Video: `video_selector` + `seedance_video`, both routed to JiubanAI's built-in Volcengine service
