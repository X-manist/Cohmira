...
When writing prompts for `video_selector` or `seedance_video`. Both tools call JiubanAI's
built-in video generation service, which owns the Volcengine endpoint, key, model, polling,
downloads, and media-library storage.

For model-specific tips, see the linked guides below.

## Runtime rule

- Use the Seedance prompting guide in `creative/prompting/seedance-prompting.md` and `.agents/skills/seedance-2-0/`.
- Do not select or request FAL, HeyGen, Kling, Runway, Veo, MiniMax, Replicate, ComfyUI, or local GPU providers.
- Keep provider credentials out of OpenMontage; JiubanAI settings are the source of truth.
- For JiubanAI's default Chinese workflow, write the actual `prompt` and `title` passed to `seedance_video` in Simplified Chinese unless the user explicitly requests another language.
- Intermediate clips belong in the media library. Only the final stitched deliverable should be surfaced in chat.
