...
Seedance is OpenMontage's video prompting target for cinematic, trailer, teaser, hype-edit, and motion-led clip work. Execution always goes through JiubanAI's built-in video service via `video_selector` or `seedance_video`; provider credentials remain in JiubanAI settings.
