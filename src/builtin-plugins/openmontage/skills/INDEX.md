...
For novel adaptation, AI comics, episodic drama, persistent characters, or story-first video, use
`pipeline_defs/ai-short-drama.yaml` and read `pipelines/ai-short-drama/executive-producer.md` first.
The host AI performs creative work from these Skills; the Python MCP runtime validates and persists
artifacts. Open `ui://openmontage/short-drama` through `drama_open_review` for all binding user choices.

| Stage | Director | Main artifacts |
|---|---|---|
| research | `ai-short-drama/research-director.md` | `source_document`, `research_brief` |
| proposal | `ai-short-drama/proposal-director.md` | `episode_plan`, `drama_bible` |
| script | `ai-short-drama/script-director.md` | `screenplay` |
| scene_plan | `ai-short-drama/scene-director.md` | `drama_storyboard`, `continuity_report` |
| assets | `ai-short-drama/asset-director.md` | `selection_manifest`, `generation_manifest`, `asset_manifest` |
| edit | `ai-short-drama/edit-director.md` | `voice_plan`, `edit_decisions` |
| compose | `ai-short-drama/compose-director.md` | `render_report`, `final_review` |
| publish | `ai-short-drama/publish-director.md` | `publish_log` |
...
| **AI Video Planning** | `seedance-2-0`, `ai-video-gen`, `video-edit`, `video-understand`, `visual-style` | Prompting and editing guidance only; execution uses JiubanAI |
| **AI Video Runtime** | `seedance_video` / `video_selector` → JiubanAI `app_cli video generate` → built-in Volcengine service | JiubanAI desktop |
| **Infrastructure** | `acestep`, `playwright-recording` | `digitalsamba/claude-code-video-toolkit` |
