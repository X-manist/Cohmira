...
class VideoTrimmer(BaseTool):
    ...
    def _cut(self, inputs: dict[str, Any]) -> ToolResult:
        ...
        if codec == "copy":
            ...
            cmd.extend(["-c:v", codec, "-preset", "fast", "-c:a", "aac"])
        cmd.append(str(output_path))

        self.run_command(cmd)
        self._validate_video_output(output_path)
            ...
            for i, seg in enumerate(segments):
                ...
                if seg_start is not None or seg_end is not None:
                    ...
                    if seg_end is not None:
                        start_value = float(seg_start or 0)
                        duration = float(seg_end) - start_value
                        if duration <= 0:
                            return ToolResult(success=False, error="segment end_seconds must be greater than start_seconds")
                        cmd.extend(["-t", str(duration)])
                    cmd.extend(["-c:v", "libx264", "-preset", "fast", "-c:a", "aac", str(temp_path)])
                    self.run_command(cmd)
                    self._validate_video_output(temp_path)
    ...
    def _validate_video_output(self, output_path: Path) -> None:
        ...
        try:
            ...
            raise RuntimeError(f"ffprobe returned invalid JSON for {output_path}: {exc}") from exc
        if not payload.get("streams"):
            raise RuntimeError(f"FFmpeg output has no video stream: {output_path}")
